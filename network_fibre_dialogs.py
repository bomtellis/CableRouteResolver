...
class PhysicalFibrePlanningDialog(QDialog):
    def __init__(self,parent=None,settings:Optional[dict]=None,cable_types:Optional[Sequence[dict]]=None):
        ...
        for label,w in [("Routing arrangement",self.mode),("Direct cable type",self.default_type),("Spine cable type",self.spine_type),("Spur cable type",self.spur_type),("Branch termination",self.branch),("Splitter termination",self.splitter),("Splitter pigtail",self.pigtail),("Maximum splices per tray",self.tray),("Spare core allowance",self.spare)]: form.addRow(label,w)
        buttons=QDialogButtonBox(QDialogButtonBox.Ok|QDialogButtonBox.Cancel); buttons.accepted.connect(self.accept); buttons.rejected.connect(self.reject); layout.addWidget(buttons)
    def accept(self):
        self.result={**self.settings,"routing_mode":_text(self.mode.currentData()),"default_cable_type_id":_text(self.default_type.currentData()),"spine_cable_type_id":_text(self.spine_type.currentData()),"spur_cable_type_id":_text(self.spur_type.currentData()),"branch_termination_method":self.branch.currentText(),"splitter_termination_method":self.splitter.currentText(),"splitter_pigtail":self.pigtail.isChecked(),"max_splices_per_cassette":int(self.tray.value()),"spare_core_percent":float(self.spare.value())}; super().accept()
