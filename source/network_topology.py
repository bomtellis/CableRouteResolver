...
def _expanded_asset_ports(asset: dict) -> List[dict]:
    ...
    for row in _asset_port_definitions(asset):
        ...
        prefix = _text(row.get("name_prefix")) or ({"pon":"PON", "sfp":"SFP", "sfp+":"SFP+", "sfp28":"SFP28", "sfp56":"SFP56", "qsfp":"QSFP", "qsfp+":"QSFP+", "qsfp28":"QSFP28", "qsfp56":"QSFP56", "qsfpdd":"QSFP-DD", "osfp":"OSFP", "lc":"LC", "sc":"SC", "mpo":"MPO", "rj45":""}.get(port_type, port_type.upper()))
...
def _port_group_sort_key(port: dict, fallback_name: str = "") -> Tuple[int, int, str, str]:
    ...
    type_order = {
        ...
        "sfp": 5, "sfp+": 6, "sfp28": 7, "sfp56": 8,
        "qsfp": 9, "qsfp+": 10, "qsfp28": 11, "qsfp56": 12,
        "qsfpdd": 13, "osfp": 14, "console": 15, "usb": 16,
        "power": 17, "other": 18,
    }
...
class TopologyModel:
    ...
    def __init__(self, data: dict, normalise: bool = True):
        ...
        self.physical_port_records_by_instance: Dict[str, Dict[str, List[str]]] = {}
        self._build()
    ...
    def _build(self) -> None:
        ...
        physical_port_records: Dict[str, Dict[str, List[str]]] = defaultdict(lambda: defaultdict(list))
        instance_names = {
        ...
        def add_physical_record(instance_id: str, ports, description: str) -> None:
            instance_id = _text(instance_id)
            if isinstance(ports, str):
                ports = [ports]
            if not instance_id or not isinstance(ports, (list, tuple, set)):
                return
            for value in ports:
                port = _text(value)
                if port and port != "0" and description not in physical_port_records[instance_id][port]:
                    physical_port_records[instance_id][port].append(description)
        ...
        for connection in self.data.get("network_connections", []):
            ...
            is_physical = bool(connection.get("topology_hidden")) or bool(connection.get("physical_connection"))
            if is_physical:
                connection_label = _text(connection.get("name")) or _text(connection.get("id")) or "Physical connection"
                source_description = f"{connection_label} to {instance_names.get(target_id, target_id or 'termination')}"
                target_description = f"{connection_label} to {instance_names.get(source_id, source_id or 'termination')}"
                add_physical_record(source_id, source_port, source_description + (f" / {target_port}" if target_port else ""))
                add_physical_record(target_id, target_port, target_description + (f" / {source_port}" if source_port else ""))
            ...
            for key in (
                ...
                if is_physical:
                    add_physical_record(source_id, connection.get(key), source_description)
            for key in (
                ...
                if is_physical:
                    add_physical_record(target_id, connection.get(key), target_description)
            for key in (
        ...
        for lead in self.data.get("network_patch_leads", []):
            ...
            lead_label = _text(lead.get("name")) or _text(lead.get("id")) or "Patch lead"
            add_physical_record(instance_id, port, f"{lead_label} to {instance_names.get(peer_instance_id, peer_instance_id or 'termination')}" + (f" / {peer_port}" if peer_port else ""))
            add_physical_record(peer_instance_id, peer_port, f"{lead_label} to {instance_names.get(instance_id, instance_id or 'termination')}" + (f" / {port}" if port else ""))
        ...
        for instance in self.data.get("network_asset_instances", []):
            ...
            for cassette in instance.get("fibre_cassettes", []):
                ...
                cassette_label = _text(cassette.get("name")) or f"Cassette {_text(cassette.get('cassette_number')) or '?'}"
                add_physical_record(instance_id, cassette.get("used_front_connector_names"), f"{cassette_label} allocated fibre termination")
        self.occupied_ports_by_instance = {
            instance_id: set(port_names)
            for instance_id, port_names in occupied_ports_by_instance.items()
        }
        self.physical_port_records_by_instance = {
            instance_id: {port: list(values) for port, values in ports.items()}
            for instance_id, ports in physical_port_records.items()
        }
        ...
        for instance in self.data.get("network_asset_instances", []):
            ...
            if asset_type == "patch_panel":
                ...
                cassette_capacity = max(
                    ...
                    * _modular_panel_front_positions(asset),
                )
                generic_port_capacity = max(
                    port_definition_capacity,
                    _int(asset.get("number_of_ports")),
    ...
    def _build_hierarchy(self) -> None:
        ...
        independent carrier services).  Descendants are attached only when the
        edge moves to a strictly downstream tier.  Any same-tier or upstream
        remainder becomes another component root so peer, standby and passive
        cross-links do not turn into the visible tree.
        """
        sorted_adjacency = {
                ...
                for child_id in remaining:
                    ...
                    for parent_id, edge in self.adjacency.get(child_id, []):
                        if parent_id not in assigned:
                            continue
                        if not self._can_descend(parent_id, child_id):
                            continue
                        parent_rank = self._hierarchy_rank(parent_id)
                        choices.append(
                            (
...
class TopologyCardItem(QGraphicsObject):
    ...
    def _paint_stack_members(self, painter: QPainter, stack_members: int) -> None:
        ...
        fibre_uplinks = max(0, _int(getattr(self.node, "connection_count", 0)))
        painter.drawText(footer_rect, Qt.AlignCenter, f"{stack_links} stack links - {fibre_uplinks} fibre uplinks")
...
class RackEquipmentItem(QGraphicsObject):
    ...
    def set_selected_port(self, node_id: str = "") -> None:
        node_id = _text(node_id)
        if self._selected_port_node_id != node_id:
            self._selected_port_node_id = node_id
            self.update()
    ...
    def _stack_member_rects(self) -> List[QRectF]:
        members = self._stack_member_count()
        if members <= 1:
            return [self.boundingRect()]
        member_slots = [
            max(1, _int(value, 1))
            for value in self.node.instance.get("stack_member_start_half_us", [])
        ]
        if len(member_slots) == members:
            first_half = min(member_slots)
            top_half = max(value + 1 for value in member_slots)
            span_half = max(2, top_half - first_half + 1)
            half_pitch = self._height / float(span_half)
            return [
                QRectF(
                    0.0,
                    (top_half - (half_slot + 1)) * half_pitch,
                    self._width,
                    2.0 * half_pitch,
                )
                for half_slot in member_slots
            ]
        gap = max(3.0, min(7.0, self._height * 0.012))
        member_h = max(10.0, (self._height - gap * (members - 1)) / members)
        return [
            QRectF(0.0, index * (member_h + gap), self._width, member_h)
            for index in range(members)
        ]

    def shape(self) -> QPainterPath:
        member_slots = self.node.instance.get("stack_member_start_half_us", [])
        if self._stack_member_count() > 1 and member_slots:
            path = QPainterPath()
            for rect in self._stack_member_rects():
    ...
    def _stack_member_for_port(self, port_node: TopologyNode, index: int) -> int:
        members = self._stack_member_count()
        if members <= 1:
            return 0
        for key in ("stack_member", "member", "switch_member"):
            member = _int(port_node.details.get(key), 0)
            if member > 0:
                return min(members - 1, member - 1)
        port_name = _text(port_node.details.get("port_name"))
        match = re.match(r"^\s*(\d+)\s*[/:\-.]", port_name)
        if match:
            member = _int(match.group(1), 1)
            return min(members - 1, max(0, member - 1))
        per_member = max(1, int(math.ceil(len(self.port_nodes) / float(members))))
        return min(members - 1, index // per_member)
    ...
    def _build_port_rects(self) -> None:
        ...
        if not self.port_nodes or self._width < 42.0 or self._height < 14.0:
            return
        if self.node.asset_type == "network_switch" and self._stack_member_count() > 1:
            self._build_stack_member_port_rects()
            return
    ...
    def _build_copper_patch_panel_port_rects(self) -> None:
        ...
        columns = 24
        half_height_panel = max(
            0,
            _int(self.node.instance.get("rack_height_half_u")),
            _int(self.node.asset.get("rack_height_half_u")),
        ) == 1
        rows = (
            max(1, int(math.ceil(len(self.port_nodes) / columns)))
            if half_height_panel
            else max(
                1,
                self.units * 2,
                int(math.ceil(len(self.port_nodes) / columns)),
            )
        )
    ...
    def _build_stack_member_port_rects(self) -> None:
        ...
        ports_by_member: Dict[int, List[TopologyNode]] = defaultdict(list)
        for index, port_node in enumerate(self.port_nodes):
            ports_by_member[self._stack_member_for_port(port_node, index)].append(port_node)

        scale = self._width / 482.6
        ear_w = min(15.0, self._width * 0.045)
        for member_index, member_ports in ports_by_member.items():
            if member_index >= len(member_rects):
                continue
            rect = member_rects[member_index]
            left = rect.left() + ear_w + max(5.0, 7.0 * scale)
            right = rect.right() - ear_w - max(5.0, 7.0 * scale)
            top = rect.top() + max(8.0, rect.height() * 0.28)
            bottom = rect.bottom() - max(3.0, rect.height() * 0.10)
            usable_w = max(8.0, right - left)
            usable_h = max(4.0, bottom - top)
            uplink_uses = {"uplink", "input", "stacking", "management", "console", "power"}
            access_ports: List[TopologyNode] = []
            uplink_ports: List[TopologyNode] = []
            for port_node in member_ports:
                port_use = _text(port_node.details.get("port_use")).lower()
                kind = self._port_kind(port_node.details.get("port_type") or port_node.details.get("port_name", ""))
                if port_use in uplink_uses or kind in {"sfp", "qsfp", "lc", "sc", "mpo"}:
                    uplink_ports.append(port_node)
                else:
                    access_ports.append(port_node)

            uplink_w = min(max(30.0, 42.0 * scale), usable_w * 0.22) if uplink_ports else 0.0
            access_right = right - uplink_w - (max(3.0, 5.0 * scale) if uplink_ports else 0.0)
            access_w = max(8.0, access_right - left)
            rows = 2
            columns = max(1, int(math.ceil(max(1, len(access_ports)) / float(rows))))
            if len(access_ports) >= 40:
                columns = 24
            gap_x = max(0.7, 1.2 * scale)
            gap_y = max(0.8, 1.4 * scale)
            port_w = max(1.8, min(7.0 * scale, (access_w - gap_x * max(0, columns - 1)) / columns))
            port_h = max(2.8, min(7.2 * scale, (usable_h - gap_y) / rows))
            block_w = columns * port_w + max(0, columns - 1) * gap_x
            block_h = rows * port_h + gap_y
            x0 = left + max(0.0, (access_w - block_w) / 2.0)
            y0 = top + max(0.0, (usable_h - block_h) / 2.0)
            for index, port_node in enumerate(access_ports):
                row = index % rows
                col = index // rows
                if col >= columns:
                    break
            ...
            if uplink_ports:
                u_cols = max(1, min(2, int(math.ceil(len(uplink_ports) / 2.0))))
                u_rows = max(1, int(math.ceil(len(uplink_ports) / float(u_cols))))
                u_gap_x = max(0.8, 1.2 * scale)
                u_gap_y = max(0.8, 1.2 * scale)
                u_w = max(3.0, min(8.0 * scale, (uplink_w - u_gap_x * max(0, u_cols - 1)) / u_cols))
                u_h = max(2.8, min(7.0 * scale, (usable_h - u_gap_y * max(0, u_rows - 1)) / u_rows))
                ux0 = access_right + max(3.0, 5.0 * scale)
                uy0 = top + max(0.0, (usable_h - (u_rows * u_h + max(0, u_rows - 1) * u_gap_y)) / 2.0)
                for index, port_node in enumerate(uplink_ports):
                    row, col = divmod(index, u_cols)
                    self._port_rects[port_node.node_id] = QRectF(
                        ux0 + col * (u_w + u_gap_x),
                        uy0 + row * (u_h + u_gap_y),
                        u_w,
    ...
    def _build_network_switch_port_rects(self) -> None:
        identity = " ".join(
            _text(value).lower()
            for value in (
                self.node.name,
                self.node.asset.get("name"),
                self.node.asset.get("model"),
                self.node.asset.get("notes"),
            )
        )
        if (
            self.units >= 4
            and len(self.port_nodes) >= 96
            and any(token in identity for token in ("modular", "chassis", "line card"))
        ):
            self._build_modular_switch_chassis_port_rects()
            return

        scale = self._width / 482.6
        ear_w = min(15.0, self._width * 0.045)
        left = ear_w + max(5.0, 7.0 * scale)
        right = self._width - ear_w - max(5.0, 7.0 * scale)
        top = max(8.0, self._height * 0.28)
        bottom = self._height - max(3.0, self._height * 0.10)
        usable_w = max(8.0, right - left)
        usable_h = max(4.0, bottom - top)
        uplink_uses = {"uplink", "input", "stacking", "management", "console", "power"}
        ...
        for port_node in self.port_nodes:
            ...
            kind = self._port_kind(port_node.details.get("port_type") or port_node.details.get("port_name", ""))
            optical_downlink = (
                port_use in {"downlink", "output", "client"}
                and kind in {"sfp", "qsfp", "lc", "sc", "mpo"}
            )
            if port_use in uplink_uses or (
                kind in {"sfp", "qsfp", "lc", "sc", "mpo"}
                and not optical_downlink
            ):
                uplink_ports.append(port_node)
            else:
                access_ports.append(port_node)
        ...
        access_right = right - uplink_w - (max(3.0, 5.0 * scale) if uplink_ports else 0.0)
        access_w = max(8.0, access_right - left)
        rows = max(2, int(math.ceil(len(access_ports) / 24.0)))
        columns = max(1, int(math.ceil(max(1, len(access_ports)) / float(rows))))
        if len(access_ports) >= 40:
            columns = 24
        gap_x = max(0.7, 1.2 * scale)
    ...
    def _build_modular_switch_chassis_port_rects(self) -> None:
        """Lay out a modular core as line-card and supervisor slots."""

        scale = self._width / 482.6
        ear_w = min(15.0, self._width * 0.045)
        left = ear_w + max(7.0, 9.0 * scale)
        right = self._width - ear_w - max(7.0, 9.0 * scale)
        top = max(14.0, self._height * 0.075)
        bottom = self._height - max(8.0, self._height * 0.045)
        usable_w = max(40.0, right - left)
        usable_h = max(30.0, bottom - top)

        downlinks: List[TopologyNode] = []
        supervisors: List[TopologyNode] = []
        for port_node in self.port_nodes:
            port_use = _text(port_node.details.get("port_use")).lower()
            if port_use in {"downlink", "output", "client"}:
                downlinks.append(port_node)
            else:
                supervisors.append(port_node)

        card_capacity = max(0, _int(self.node.asset.get("line_card_port_capacity"), 0))
        if card_capacity <= 1:
            note = _text(self.node.asset.get("notes")).lower()
            match = re.search(r"(\d+)\s*[- ]port\s+(?:fibre\s+)?line card", note)
            card_capacity = max(0, _int(match.group(1), 0)) if match else 0
        if card_capacity <= 1:
            card_capacity = (
                56
                if len(downlinks) >= 112 and len(downlinks) % 56 == 0
                else 48
            )
        card_count = max(1, int(math.ceil(len(downlinks) / float(card_capacity))))
        supervisor_slot_count = 2 if supervisors else 0
        power_zone_h = max(18.0, usable_h * 0.16)
        gap = max(2.0, 3.0 * scale)
        slot_count = card_count + supervisor_slot_count
        slot_h = max(
            18.0,
            (
                usable_h
                - power_zone_h
                - gap * max(0, slot_count)
            )
            / max(1, slot_count),
        )

        def place_ports(
            ports: Sequence[TopologyNode], module_rect: QRectF, columns: int
        ) -> None:
            label_h = max(10.0, min(16.0, module_rect.height() * 0.08))
            grid = module_rect.adjusted(5.0, label_h + 4.0, -5.0, -6.0)
            columns = max(1, min(columns, len(ports) or 1))
            rows = max(1, int(math.ceil(len(ports) / float(columns))))
            gap_x = max(0.8, 1.2 * scale)
            gap_y = max(0.8, 1.2 * scale)
            port_w = max(
                2.4,
                (grid.width() - gap_x * max(0, columns - 1)) / columns,
            )
            port_h = max(
                2.4,
                min(
                    8.0,
                    (grid.height() - gap_y * max(0, rows - 1)) / rows,
                ),
            )
            block_h = rows * port_h + gap_y * max(0, rows - 1)
            y0 = grid.top() + max(0.0, (grid.height() - block_h) / 2.0)
            for index, port_node in enumerate(ports):
                row, col = divmod(index, columns)
                self._port_rects[port_node.node_id] = QRectF(
                    grid.left() + col * (port_w + gap_x),
                    y0 + row * (port_h + gap_y),
                    port_w,
                    port_h,
                )

        ordered_modules: List[Tuple[str, Sequence[TopologyNode], int]] = []
        configured = self.node.asset.get("chassis_modules", [])
        if not isinstance(configured, list) or not configured:
            configured = [
                {"slot": index + 1, "module_type": "line_card", "model": "Line card"}
                for index in range(card_count)
            ]
        down_cursor = 0
        up_cursor = 0
        for module in sorted(
            (row for row in configured if isinstance(row, dict)),
            key=lambda row: _int(row.get("slot"), 0),
        ):
            slot = max(1, _int(module.get("slot"), len(ordered_modules) + 1))
            model = _text(module.get("model"))
            module_type = _text(module.get("module_type")).lower()
            if module_type == "supervisor":
                ordered_modules.append((f"SLOT {slot}  {model or 'SUPERVISOR'}", [], 1))
                continue
            definitions = CATALYST_9600_LINE_CARDS.get(model, [])
            down_count = sum(count for _kind, count, use, _speeds in definitions if use == "downlink")
            up_count = sum(count for _kind, count, use, _speeds in definitions if use == "uplink")
            if not definitions:
                down_count = card_capacity
                up_count = int(math.ceil(len(supervisors) / float(max(1, card_count))))
            ports = list(downlinks[down_cursor : down_cursor + down_count])
            ports.extend(supervisors[up_cursor : up_cursor + up_count])
            down_cursor += down_count
            up_cursor += up_count
            ordered_modules.append((f"SLOT {slot}  {model or 'LINE CARD'}", ports, 30))

        cursor_y = top
        for module_name, module_ports, columns in ordered_modules:
            module_rect = QRectF(left, cursor_y, usable_w, slot_h)
            self._switch_module_rects.append((module_name, module_rect))
            place_ports(module_ports, module_rect, columns)
            cursor_y += slot_h + gap
        ...
        power_rect = QRectF(
            ...
            cursor_y,
            usable_w,
            max(12.0, bottom - cursor_y),
        )
        self._switch_module_rects.append(("POWER SUPPLIES / FAN TRAY", power_rect))

    def _build_modular_patch_panel_port_rects(self) -> None:
        scale = self._width / 482.6
        ear_w = min(15.0, self._width * 0.045)
        left = ear_w + max(4.0, 6.0 * scale)
        right = self._width - ear_w - max(4.0, 6.0 * scale)
        top = max(11.0, self._height * 0.18)
        bottom = self._height - max(4.0, self._height * 0.09)
        usable_w = max(20.0, right - left)
        usable_h = max(10.0, bottom - top)
        cassette_count = max(1, _int(self.node.asset.get("patch_panel_cassette_count"), 1))
        cassette_capacity = _modular_panel_front_positions(self.node.asset)
        cassette_rows = max(1, _int(self.node.asset.get("patch_panel_cassette_rows"), int(math.ceil(cassette_count / 4.0))))
        cassette_cols = max(1, _int(self.node.asset.get("patch_panel_cassette_columns"), min(4, cassette_count)))
        while cassette_rows * cassette_cols < cassette_count:
            cassette_rows += 1
        gap_x = max(3.0, 5.0 * scale)
        gap_y = max(2.2, 3.5 * scale)
        cassette_w = max(8.0, (usable_w - gap_x * max(0, cassette_cols - 1)) / cassette_cols)
        cassette_h = max(7.0, (usable_h - gap_y * max(0, cassette_rows - 1)) / cassette_rows)
        port_rows = max(1, _int(self.node.asset.get("patch_panel_cassette_port_rows"), int(math.ceil(cassette_capacity / 12.0))))
        port_cols = max(1, _int(self.node.asset.get("patch_panel_cassette_port_columns"), min(12, cassette_capacity)))
        while port_rows * port_cols < cassette_capacity:
            port_rows += 1
        ...
        for port_node in self.port_nodes:
            ...
            match = re.match(r"c(\d+)[-_/ ]", name.lower())
            if match:
                cassette = max(1, _int(match.group(1), 1))
            else:
                cassette = fallback_index // cassette_capacity + 1
                fallback_index += 1
            ports_by_cassette[cassette].append(port_node)

        for cassette in range(1, cassette_count + 1):
            cassette_index = cassette - 1
            row, col = divmod(cassette_index, cassette_cols)
            cassette_rect = QRectF(
                left + col * (cassette_w + gap_x),
                top + row * (cassette_h + gap_y),
                cassette_w,
                cassette_h,
            )
            ...
            port_gap_x = max(0.6, min(2.0, cassette_w * 0.015))
            port_gap_y = max(0.6, min(2.0, cassette_h * 0.035))
            port_w = max(2.8, (grid.width() - port_gap_x * max(0, port_cols - 1)) / port_cols)
            port_h = max(2.8, (grid.height() - port_gap_y * max(0, port_rows - 1)) / port_rows)
            for index, port_node in enumerate(ports_by_cassette.get(cassette, [])[:cassette_capacity]):
                port_row, port_col = divmod(index, port_cols)
                if port_row >= port_rows:
                    break
                self._port_rects[port_node.node_id] = QRectF(
                    grid.left() + port_col * (port_w + port_gap_x),
                    grid.top() + port_row * (port_h + port_gap_y),
                    port_w,
                    port_h,
                )

    def _paint_stack_member_faces(self, painter: QPainter, ear_w: float) -> None:
        member_rects = self._stack_member_rects()
        members = len(member_rects)
        name = QFontMetrics(QFont("Arial", 6)).elidedText(
            self.node.name,
            Qt.ElideRight,
            max(20, int(self._width - 2 * ear_w - 74.0)),
        )
        for index, member_rect in enumerate(member_rects, start=1):
            face = member_rect.adjusted(1.8, 1.2, -1.8, -1.2)
            painter.setPen(QPen(QColor("#71808b"), 0.9))
            painter.setBrush(QColor("#26323c"))
            painter.drawRoundedRect(face, 2.5, 2.5)
            painter.setBrush(QColor("#151c22"))
            painter.drawRect(QRectF(face.left(), face.top() + 1.0, ear_w, max(2.0, face.height() - 2.0)))
            painter.drawRect(QRectF(face.right() - ear_w, face.top() + 1.0, ear_w, max(2.0, face.height() - 2.0)))
            label_font = QFont("Arial", 5 if face.height() < 34.0 else 6)
            ...
            label_rect = QRectF(
                face.left() + ear_w + 3.0,
                face.top() + 1.0,
                max(12.0, face.width() - 2 * ear_w - 6.0),
                min(10.0, face.height() * 0.28),
            )
            painter.drawText(
                label_rect,
                Qt.AlignLeft | Qt.AlignVCenter,
                f"SW{index}/{members}  {name}",
            )
    ...
    def paint(self, painter: QPainter, option, widget=None) -> None:
        ...
        ear_w = min(15.0, self._width * 0.045)
        spaced_stack = bool(
            self.node.asset_type == "network_switch"
            and self._stack_member_count() > 1
            and self.node.instance.get("stack_member_start_half_us")
        )
        if not spaced_stack:
        ...
        if self.node.asset_type == "network_switch" and self._stack_member_count() > 1:
            self._paint_stack_member_faces(painter, ear_w)
            if spaced_stack and (selected or self.search_match):
                painter.setBrush(Qt.NoBrush)
                painter.setPen(QPen(border, 2.0))
                for member_rect in self._stack_member_rects():
                    painter.drawRoundedRect(member_rect.adjusted(0.8, 0.8, -0.8, -0.8), 2.5, 2.5)
        else:
            painter.setPen(QColor('#dce5ea'))
        ...
        for module_name, module_rect in self._switch_module_rects:
            painter.setBrush(QColor("#1b252d"))
            painter.setPen(QPen(QColor("#60717e"), 1.0))
            painter.drawRoundedRect(module_rect, 2.5, 2.5)
            module_font = QFont("Arial", 5 if module_rect.height() < 28 else 6)
            module_font.setBold(True)
            painter.setFont(module_font)
            painter.setPen(QColor("#b8c8d4"))
            painter.drawText(
                module_rect.adjusted(3.0, 2.0, -3.0, 0.0),
        ...
        for port_node in self.port_nodes:
            ...
            if port_node.node_id == self._selected_port_node_id:
                painter.setBrush(Qt.NoBrush)
                painter.setPen(QPen(QColor("#f5bf42"), max(1.4, port_rect.width() * 0.12)))
                painter.drawRoundedRect(port_rect.adjusted(-1.8, -1.8, 1.8, 1.8), 2.0, 2.0)
        ...
        if event.button() == Qt.LeftButton:
            ...
            for node_id, rect in self._port_rects.items():
                if rect.adjusted(-1.5, -1.5, 1.5, 1.5).contains(point):
                    ...
                    self._drag_active = False
                    self._drag_moving = False
                    self._port_press_active = True
                    self.set_selected_port(node_id)
...
class SwitchFrontPanelItem(QGraphicsObject):
    ...
    def _build_network_switch_port_rects(self) -> None:
        ...
        bottom_margin = 12.0
        usable_w = max(20.0, self._width - margin_x * 2.0)
        usable_h = max(10.0, self._height - title_h - bottom_margin)
        uplink_w = min(max(86.0, self._width * 0.12), usable_w * 0.22) if uplink_ports else 0.0
        access_gap = 18.0 if uplink_ports else 0.0
        access_w = max(20.0, usable_w - uplink_w - access_gap)

        rows = max(2, int(math.ceil(len(access_ports) / 24.0)))
        columns = max(1, int(math.ceil(max(1, len(access_ports)) / float(rows))))
        if len(access_ports) >= 40:
            columns = 24
        gap_x = max(2.0, self._width * 0.0024)
        gap_y = max(5.0, self._height * 0.045)
        port_w = max(4.0, (access_w - gap_x * max(0, columns - 1)) / columns)
        port_h = max(5.0, min(15.0, (usable_h - gap_y) / rows))
        block_w = columns * port_w + max(0, columns - 1) * gap_x
        block_h = rows * port_h + gap_y
        x0 = margin_x + max(0.0, (access_w - block_w) / 2.0)
        y0 = title_h + max(0.0, (usable_h - block_h) / 2.0)
        for index, port_node in enumerate(access_ports):
            row = index % rows
            col = index // rows
            if col >= columns:
        ...
        if uplink_ports:
            u_cols = max(1, min(4, int(math.ceil(len(uplink_ports) / 2.0))))
            u_rows = max(1, int(math.ceil(len(uplink_ports) / float(u_cols))))
            u_gap_x = max(2.0, self._width * 0.002)
            u_gap_y = max(3.0, self._height * 0.03)
            u_w = max(5.0, (uplink_w - u_gap_x * max(0, u_cols - 1)) / u_cols)
            u_h = max(4.5, min(13.0, (usable_h - u_gap_y * max(0, u_rows - 1)) / u_rows))
            ux0 = margin_x + access_w + access_gap
            uy0 = title_h + max(0.0, (usable_h - (u_rows * u_h + max(0, u_rows - 1) * u_gap_y)) / 2.0)
            for index, port_node in enumerate(uplink_ports):
                row, col = divmod(index, u_cols)
                self._port_rects[port_node.node_id] = QRectF(
                    ux0 + col * (u_w + u_gap_x),
    ...
    def _build_modular_patch_panel_port_rects(self) -> None:
        ...
        bottom_margin = 10.0
        usable_w = max(20.0, self._width - margin_x * 2.0)
        usable_h = max(14.0, self._height - top - bottom_margin)
        cassette_count = max(1, _int(self.node.asset.get("patch_panel_cassette_count"), 1))
        cassette_capacity = _modular_panel_front_positions(self.node.asset)
        cassette_rows = max(1, _int(self.node.asset.get("patch_panel_cassette_rows"), int(math.ceil(cassette_count / 4.0))))
        cassette_cols = max(1, _int(self.node.asset.get("patch_panel_cassette_columns"), min(4, cassette_count)))
        while cassette_rows * cassette_cols < cassette_count:
            cassette_rows += 1
        gap_x = max(5.0, self._width * 0.008)
        gap_y = max(4.0, self._height * 0.018)
        cassette_w = max(12.0, (usable_w - gap_x * max(0, cassette_cols - 1)) / cassette_cols)
        cassette_h = max(10.0, (usable_h - gap_y * max(0, cassette_rows - 1)) / cassette_rows)
        port_rows = max(1, _int(self.node.asset.get("patch_panel_cassette_port_rows"), int(math.ceil(cassette_capacity / 12.0))))
        port_cols = max(1, _int(self.node.asset.get("patch_panel_cassette_port_columns"), min(12, cassette_capacity)))
        while port_rows * port_cols < cassette_capacity:
            port_rows += 1
        ...
        for cassette in range(1, cassette_count + 1):
            ...
            grid = inner.adjusted(0.0, label_h, 0.0, 0.0)
            port_gap_x = max(1.0, min(3.0, cassette_w * 0.018))
            port_gap_y = max(1.0, min(3.0, cassette_h * 0.035))
            port_w = max(4.0, (grid.width() - port_gap_x * max(0, port_cols - 1)) / port_cols)
            port_h = max(4.0, (grid.height() - port_gap_y * max(0, port_rows - 1)) / port_rows)
            for index, port_node in enumerate(ports_by_cassette.get(cassette, [])[:cassette_capacity]):
                port_row, port_col = divmod(index, port_cols)
                if port_row >= port_rows:
                    break
...
class SplitterFrontPanelItem(QGraphicsObject):
    ...
    def paint(self, painter: QPainter, option, widget=None) -> None:
        ...
        for port_node in self.port_nodes:
            ...
            if port_node.node_id == self._selected_port_node_id:
                painter.setBrush(Qt.NoBrush)
                painter.setPen(QPen(QColor("#f5bf42"), max(1.4, pr.width() * 0.12)))
                painter.drawRoundedRect(pr.adjusted(-1.8, -1.8, 1.8, 1.8), 2.0, 2.0)
...
class TopologyGraphicsView(QGraphicsView):
    ...
    def mouseDoubleClickEvent(self, event) -> None:  # noqa: ANN001
        ...
        if event.button() == Qt.LeftButton:
            ...
            if self._edge_id_at(position):
                ...
                self._request_full_repaint()
                return
            # Rack frames and U-grid artwork are passive scene items.  They
            # must behave like empty rack space for the add-device gesture.
            if self._interactive_item_at(position) is None:
    ...
    def _handle_left_press(
        ...
        # A passive item was clicked. It has no scene-level mouse behavior.
        # Consume both press and release in the view, but keep the graphics
        # scene interactive; toggling QGraphicsView interaction while
        # non-selectable paths/text/backgrounds are under the mouse can leave
        # those passive items out of the next repaint on some viewports.
...
class CommsRoomIsometricDialog(QDialog):
    """Read-only 2.5D location view showing rack positions and mounted kit."""

    def __init__(self, owner, floor: int, location: str, parent: Optional[QWidget] = None):
        super().__init__(parent)
        self.owner = owner
        self.floor = int(floor)
        self.location = _text(location)
        self.setWindowTitle(f"Comms room layout - {self.location or 'Unassigned'} F{self.floor}")
    ...
    def _fit(self) -> None:
        rect = self.scene.itemsBoundingRect().adjusted(-80.0, -70.0, 80.0, 90.0)
        if rect.isEmpty():
            return
        self.view.fitInView(rect, Qt.KeepAspectRatio)

    @staticmethod
    def _equipment_colour(node: TopologyNode) -> QColor:
        asset_type = _text(node.asset_type).lower()
        role = _text(node.role).lower()
        if asset_type == "network_switch":
            return QColor("#2b6fa3") if "access" in role else QColor("#3560a8")
        if asset_type in {"patch_panel", "fibre_splitter"}:
            return QColor("#62717d")
        if asset_type == "optical_line_terminal":
            return QColor("#4f61b8")
        if asset_type in {"network_router", "firewall"}:
            return QColor("#9b5e52")
        if asset_type == "pdu":
            return QColor("#6e3c86")
        if asset_type in {"ups", "power_device"}:
            return QColor("#866d36")
        return QColor("#3f515d")

    @staticmethod
    def _short_label(text: str, width: float, font: QFont) -> str:
        return QFontMetrics(font).elidedText(_text(text), Qt.ElideRight, max(12, int(width)))

    def _rack_nodes(self, rack_name: str) -> List[TopologyNode]:
        node_ids = self.owner._rack_nodes_by_location.get((self.floor, self.location), [])
        nodes = [
            self.owner.model.nodes[node_id]
            for node_id in node_ids
            if node_id in self.owner.model.nodes
            and _text(self.owner.model.nodes[node_id].instance.get("rack_name")) == rack_name
        ]
        return sorted(
            nodes,
            key=lambda node: (
                max(1, _int(node.instance.get("rack_start_u"), 1)),
                node.name.lower(),
            ),
        )

    def _draw_rack(self, rack_name: str, x: float, y: float) -> QRectF:
        key = (self.floor, self.location, rack_name)
        capacity = max(1, self.owner._rack_capacity_for_key(key))
        used = max(0, self.owner._rack_used_for_key(key))
        front_w = 118.0
        front_h = max(210.0, min(310.0, capacity * 7.0))
        skew_x = 36.0
        skew_y = -24.0

        front = QRectF(x, y, front_w, front_h)
        top_path = QPainterPath(QPointF(front.left(), front.top()))
        top_path.lineTo(QPointF(front.left() + skew_x, front.top() + skew_y))
        top_path.lineTo(QPointF(front.right() + skew_x, front.top() + skew_y))
        top_path.lineTo(QPointF(front.right(), front.top()))
        top_path.closeSubpath()
        side_path = QPainterPath(QPointF(front.right(), front.top()))
        side_path.lineTo(QPointF(front.right() + skew_x, front.top() + skew_y))
        side_path.lineTo(QPointF(front.right() + skew_x, front.bottom() + skew_y))
        side_path.lineTo(QPointF(front.right(), front.bottom()))
        side_path.closeSubpath()
        front_path = QPainterPath()
        front_path.addRoundedRect(front, 5.0, 5.0)

        for path, fill, z in (
            (top_path, QColor("#27343d"), 0.0),
            (side_path, QColor("#1b252d"), 0.1),
            (front_path, QColor("#202b34"), 0.2),
        ):
            item = QGraphicsPathItem(path)
            item.setPen(QPen(QColor("#536371"), 1.1))
            item.setBrush(fill)
            item.setZValue(z)
            item.setAcceptedMouseButtons(Qt.NoButton)
            self.scene.addItem(item)

        pitch = front_h / float(capacity)
        for step in range(0, capacity + 1, 4):
            yy = front.bottom() - step * pitch
            line = QPainterPath(QPointF(front.left() + 5.0, yy))
            line.lineTo(QPointF(front.right() - 5.0, yy))
            item = QGraphicsPathItem(line)
            item.setPen(QPen(QColor("#33434e"), 0.45))
            item.setZValue(0.25)
            item.setAcceptedMouseButtons(Qt.NoButton)
            self.scene.addItem(item)
        ...
        font = QFont("Arial", 5)
        font.setBold(True)
        for node in self._rack_nodes(rack_name):
            start_u = max(1, _int(node.instance.get("rack_start_u"), 1))
            entries: List[Tuple[int, int, str]] = []
            if (
                node.asset_type == "network_switch"
                and bool(node.instance.get("logical_stack"))
                and _int(node.instance.get("stack_member_count"), 1) > 1
            ):
                members = max(1, _int(node.instance.get("stack_member_count"), 1))
                member_units = max(
                    1,
                    _int(
                        node.asset.get("switch_rack_unit_allowance"),
                        _int(node.asset.get("rack_units"), 1),
                    ),
                )
                for member in range(1, members + 1):
                    entries.append(
                        (
                            start_u + (member - 1) * member_units,
                            member_units,
                            f"{node.name} SW{member}",
                        )
                    )
            else:
                entries.append((start_u, max(1, self.owner._node_rack_units(node.node_id)), node.name))
            for item_start_u, units, label_text in entries:
                if item_start_u > capacity:
                    continue
                top_u = min(capacity, item_start_u + units - 1)
                item_y = front.bottom() - top_u * pitch + 1.0
                item_h = max(5.0, units * pitch - 2.0)
                item_rect = QRectF(front.left() + 8.0, item_y, front.width() - 16.0, item_h)
                path = QPainterPath()
                path.addRoundedRect(item_rect, 2.2, 2.2)
                equipment = QGraphicsPathItem(path)
                equipment.setPen(QPen(self._equipment_colour(node).lighter(130), 0.8))
                equipment.setBrush(self._equipment_colour(node))
                equipment.setZValue(0.45)
                equipment.setToolTip(f"{label_text}\nU{item_start_u} - {units}U")
                self.scene.addItem(equipment)
                if item_h >= 10.0:
                    label = QGraphicsSimpleTextItem(self._short_label(label_text, item_rect.width() - 6.0, font))
                    label.setFont(font)
                    label.setBrush(QBrush(QColor("#eef5f8")))
                    label.setPos(item_rect.left() + 3.0, item_rect.top() + max(0.0, (item_h - label.boundingRect().height()) / 2.0))
                    label.setZValue(0.6)
        ...
        label_font.setBold(True)
        label = QGraphicsSimpleTextItem(f"{rack_name}  {used}/{capacity}U")
        label.setFont(label_font)
        label.setBrush(QBrush(QColor("#e4edf2")))
        label.setPos(front.left(), front.bottom() + 10.0)
        label.setZValue(1.0)
        label.setAcceptedMouseButtons(Qt.NoButton)
        self.scene.addItem(label)
        return front.united(QRectF(front.right(), front.top() + skew_y, skew_x, front_h))

    def _draw_room(self) -> None:
        rack_names = self.owner._rack_names_for_location(self.floor, self.location)
        title = QGraphicsSimpleTextItem(f"{self.location or 'Unassigned location'} - floor {self.floor}")
        title_font = QFont("Arial", 13)
        title_font.setBold(True)
        title.setFont(title_font)
        title.setBrush(QBrush(QColor("#f1f6f8")))
        title.setPos(24.0, 20.0)
        title.setAcceptedMouseButtons(Qt.NoButton)
        self.scene.addItem(title)

        if not rack_names:
            empty = QGraphicsSimpleTextItem("No racks are defined for this location.")
            empty.setFont(QFont("Arial", 10))
            empty.setBrush(QBrush(QColor("#9aa8b3")))
            empty.setPos(24.0, 66.0)
            self.scene.addItem(empty)
            self.scene.setSceneRect(self.scene.itemsBoundingRect().adjusted(-40.0, -40.0, 40.0, 40.0))
            return

        x = 42.0
        y = 104.0
        gap = 72.0
        union: Optional[QRectF] = None
        for rack_name in rack_names:
            rect = self._draw_rack(rack_name, x, y)
            union = QRectF(rect) if union is None else union.united(rect)
            x += rect.width() + gap

        if union is not None:
            floor_path = QPainterPath()
            floor_rect = union.adjusted(-34.0, 34.0, 54.0, 70.0)
            floor_path.moveTo(floor_rect.left(), floor_rect.bottom())
            floor_path.lineTo(floor_rect.left() + 44.0, floor_rect.top())
            floor_path.lineTo(floor_rect.right(), floor_rect.top())
            floor_path.lineTo(floor_rect.right() - 44.0, floor_rect.bottom())
            floor_path.closeSubpath()
            floor_item = QGraphicsPathItem(floor_path)
            floor_item.setPen(QPen(QColor("#283742"), 1.0))
            floor_item.setBrush(QColor("#151e25"))
            floor_item.setZValue(-1.0)
            floor_item.setAcceptedMouseButtons(Qt.NoButton)
            self.scene.addItem(floor_item)

        self.scene.setSceneRect(self.scene.itemsBoundingRect().adjusted(-70.0, -70.0, 90.0, 110.0))
...
class NetworkTopologyDialog(QDialog):
    ...
    def __init__(
        ...
        self._room_view_dialogs: List[CommsRoomIsometricDialog] = []
    ...
    def _build_details_panel(self) -> QWidget:
        ...
        self.room_view_button = QPushButton("Open comms room view")
        self.room_view_button.setToolTip("Show a 2.5D rack-row view of the selected location")
        self.room_view_button.setEnabled(False)
        self.room_view_button.clicked.connect(self._open_selected_room_view)
        panel_layout.addWidget(self.room_view_button)
    ...
    def _detail_link_list(self, label: str, links: Sequence[dict]) -> None:
        """Add readable, one-per-line links to devices and physical ports."""
        row = QWidget()
        layout = QHBoxLayout(row)
        layout.setContentsMargins(0, 3, 0, 3)
        key = QLabel(label)
        ...
        for record in links:
            text = _text(record.get("text")) or "Connection"
            target_id = _text(record.get("target_id"))
            target_port = _text(record.get("target_port"))
            item = QLabel()
            item.setWordWrap(True)
            if target_id:
                href = "network-asset:" + quote(target_id, safe="")
                if target_port:
                    href += "|" + quote(target_port, safe="")
                item.setText(f'<a href="{href}">{escape(text)}</a>')
                item.setTextInteractionFlags(Qt.TextBrowserInteraction)
                item.setOpenExternalLinks(False)
                item.linkActivated.connect(self._navigate_detail_link)
                item.setStyleSheet("color: #0b62a4;")
            else:
                item.setText(escape(text))
                item.setTextInteractionFlags(Qt.TextSelectableByMouse)
                item.setStyleSheet("color: #343a40;")
            values_layout.addWidget(item)
        layout.addWidget(key, 0, Qt.AlignTop)
        layout.addWidget(values, 1)
        self.details_layout.addWidget(row)

    def _navigate_detail_link(self, href: str) -> None:
        value = _text(href)
        if not value.startswith("network-asset:"):
            return
        payload = value[len("network-asset:"):]
        encoded_id, separator, encoded_port = payload.partition("|")
        instance_id = unquote(encoded_id)
        port_name = unquote(encoded_port) if separator else ""
        node = self.model.nodes.get(instance_id)
        if node is None:
            return
        ...
        if item is not None:
            ...
            self._show_node_details(instance_id)
    ...
    def _show_node_details(self, node_id: str) -> None:
        ...
        if not node.pseudo:
            ...
            if node.role == "switch_port":
                ...
                if not any(key in node.details for key in ("fibre_cable_details", "rack_patch_details")):
                    parent_instance_id = _text(node.details.get("parent_instance_id"))
                    port_name = _text(node.details.get("port_name"))
                    if parent_instance_id and port_name:
                        node.details.update(self._port_connection_details(parent_instance_id, port_name))
                fibre_details = node.details.get("fibre_cable_details", []) or []
                if fibre_details:
                    if all(isinstance(value, dict) for value in fibre_details):
                        self._detail_link_list("Fibre cable", fibre_details)
                    else:
                        self._detail_row("Fibre cable", "\n".join(str(value) for value in fibre_details))
                patch_details = node.details.get("rack_patch_details", []) or []
                if patch_details:
                    if all(isinstance(value, dict) for value in patch_details):
                        self._detail_link_list("Rack patch", patch_details)
                    else:
                        self._detail_row("Rack patch", "\n".join(str(value) for value in patch_details))
        ...
        for neighbour_id, edge in self.model.adjacency.get(node.node_id, []):
            ...
            peer_port = (
                ...
                else edge.source_port
            )
            connected[-1] = {
                "text": (
                    f"{neighbour.name} — {edge.medium.title()}{suffix}"
                    + (f" — port {peer_port}" if peer_port else "")
                ),
        ...
        if connected:
            ...
            self._detail_link_list("", connected[:30])
            if len(connected) > 30:
        ...
        room_key = self._room_group_key(node.node_id)
        self.room_view_button.setEnabled(not node.pseudo and room_key != (0, ""))
        self.room_view_button.setProperty("node_id", node.node_id)
    ...
    def _build_runtime_indexes(self) -> None:
        ...
        occupied_half_units: Dict[Tuple[int, str, str], Set[int]] = defaultdict(set)
        ...
        for node_id, node in self.model.nodes.items():
            ...
            half_units = self._node_rack_half_units(node_id)
            if half_units <= 0:
                continue
            start_half_u = self._node_rack_start_half_u(node)
            capacity = self._rack_capacity_cache[rack_key]
            for rack_half_u in range(start_half_u, start_half_u + half_units):
                if 1 <= rack_half_u <= capacity * 2:
                    occupied_half_units[rack_key].add(rack_half_u)

        occupied_keys = set(occupied_half_units) | set(self._pdu_nodes_by_rack)
        ...
        self._rack_used_cache = {
            key: int(math.ceil(len(values) / 2.0))
            for key, values in occupied_half_units.items()
        }
    def _defined_ports(self, node: TopologyNode) -> List[dict]:
        ...
        for member in range(1, members + 1):
            for row in ports:
                ...
                expanded.append({**row, "name": f"{member}/{name}" if name else str(member)})
        return expanded
    ...
    @staticmethod
    def _stack_member_count(node: TopologyNode) -> int:
        if node.asset_type != "network_switch" or not bool(node.instance.get("logical_stack")):
            return 1
        return max(1, _int(node.instance.get("stack_member_count"), 1))

    @staticmethod
    def _port_stack_member_number(port_node: TopologyNode) -> int:
        for key in ("stack_member", "member", "switch_member"):
            member = _int(port_node.details.get(key), 0)
            if member > 0:
                return member
    ...
    def _instance_rack_description(self, instance_id: str, port_name: str = "") -> str:
        node = self.model.nodes.get(_text(instance_id))
        if node is None:
            return _text(instance_id)
        parts = [node.name]
        if port_name:
        ...
        start_u = _int(node.instance.get("rack_start_u"))
        if rack:
            parts.append(rack + (f" U{start_u}" if start_u else ""))
        return " - ".join(parts)

    def _port_connection_details(self, instance_id: str, port_name: str) -> dict:
        instance_id = _text(instance_id)
        port_name = _text(port_name)
        fibre_rows: List[dict] = []
        ...
        for connection in self.data.get("network_connections", []):
            ...
            if from_id == instance_id and from_port == port_name:
                peer_id, peer_port = to_id, to_port
            elif to_id == instance_id and to_port == port_name:
                peer_id, peer_port = from_id, from_port
            else:
                continue
            connection_id = _text(connection.get("id"))
            if connection_id:
                logical_ids.add(connection_id)
            medium = _text(connection.get("medium")).lower()
            length_m = max(0.0, _float(connection.get("length_m")), _float(connection.get("route_length_m")), _float(connection.get("cable_length_m")), _float(connection.get("calculated_length_m")), _float(connection.get("total_length_m")))
            label = _text(connection.get("label")) or connection_id or "Connection"
            row = f"{label}: {self._instance_rack_description(peer_id, peer_port)}"
            if length_m:
                row += f" ({length_m:.1f} m)"
            if medium in {"fibre", "fiber", "optical"}:
                fibre_rows.append(
                    {"text": row, "target_id": peer_id, "target_port": peer_port}
                )
            else:
                patch_rows.append(
                    {
                        "text": f"{medium.title() or 'Connection'} {row}",
                        "target_id": peer_id,
                        "target_port": peer_port,
                    }
                )
        ...
        for cable in self.data.get("network_fibre_cables", []):
            if not isinstance(cable, dict):
                continue
            cable_logical = {_text(value) for value in cable.get("logical_connection_ids", []) if _text(value)}
            from_id = _text(cable.get("from_instance_id"))
            to_id = _text(cable.get("to_instance_id"))
            from_port = _text(cable.get("from_port"))
            to_port = _text(cable.get("to_port"))
            direct_from = from_id == instance_id and from_port == port_name
            direct_to = to_id == instance_id and to_port == port_name
            if not direct_from and not direct_to and not (logical_ids and logical_ids.intersection(cable_logical)):
                continue
            peer_id = to_id if direct_from or from_id == instance_id else from_id
            peer_port = to_port if direct_from or from_id == instance_id else from_port
            cable_id = _text(cable.get("id"))
            name = _text(cable.get("name")) or cable_id or "Fibre cable"
            extras = []
            core_count = _int(cable.get("core_count"))
            length_m = max(0.0, _float(cable.get("length_m")), _float(cable.get("calculated_length_m")))
            if core_count:
                extras.append(f"{core_count}F")
            if length_m:
                extras.append(f"{length_m:.1f} m")
            fibre_rows.append(
                {
                    "text": (
                        f"{name}{f' [{cable_id}]' if cable_id and cable_id != name else ''}: "
                        f"{self._instance_rack_description(peer_id, peer_port)}"
        ...
        for lead in self.data.get("network_patch_leads", []):
            ...
            peer_id = _text(lead.get("peer_instance_id"))
            local_port = _text(lead.get("port"))
            peer_port = _text(lead.get("peer_port"))
            if local_id == instance_id and local_port == port_name:
                target_id, target_port = peer_id, peer_port
            elif peer_id == instance_id and peer_port == port_name:
                target_id, target_port = local_id, local_port
            else:
                continue
            patch_rows.append(
                {
                    "text": (
                        f"{_text(lead.get('id')) or 'Patch cable'}: "
                        f"{(_text(lead.get('medium')) or 'patch').title()} to "
                        f"{self._instance_rack_description(target_id, target_port)}"
                    ),
                    "target_id": target_id,
        ...
        return {
            "fibre_cable_details": fibre_rows,
            "rack_patch_details": patch_rows,
        }
    ...
    def _collect_switch_port_visible(self) -> List[str]:
        ...
        for index, port in enumerate(ordered_names, start=1):
            ...
            port_definition = port_definitions_by_name.get(port) or _port_definition_for_name(switch.asset, port)
            node = TopologyNode(
                node_id=node_id,
                name=f"Port {port}",
    ...
    def _rack_port_nodes(self, device_id: str) -> List[TopologyNode]:
        ...
        for index, port in enumerate(ordered_names, start=1):
            ...
            port_definition = port_definitions_by_name.get(port) or _port_definition_for_name(device.asset, port)
    ...
    def _space_recursive_subtrees(
        ...
        fixed_bus_nodes = set(getattr(self, "_location_bus_by_node", {})) | set(
            getattr(self, "_failover_bus_by_node", {})
        )

        def place(node_id: str, band_top: float) -> None:
            node_band = band_height.get(node_id, self._node_card_height(node_id))
            own_height = self._node_card_height(node_id)
            if node_id not in fixed_bus_nodes:
    ...
    def _layout_rack_visible(self, visible_ids: Sequence[str]) -> Dict[str, QPointF]:
        ...
        for node_id in visible_ids:
            ...
            start_half_u = self._node_rack_start_half_u(node)
            half_units = max(1, self._node_rack_half_units(node_id))
            ...
            top_half_u = min(capacity * 2, start_half_u + half_units - 1)
            y = rack_top + (capacity * 2 - top_half_u) * (unit_pitch / 2.0)
    ...
    def rebuild_scene(self, fit: bool = False) -> None:
        ...
        for child_id, parent_id in self.visible_parent.items():
            ...
            if key not in seen_drawn_fibres:
                ...
                if self._edge_is_failover(edge) or self._edge_has_parallel_secondary(edge, child_id):
                    self._visible_failover_bus_nodes.add(child_id)
        ...
        if self.switch_port_focus is not None:
            ...
            if node is not None and switch_id in positions:
                ...
                panel_width = 965.2
                stack_members = self._stack_member_count(node)

                def add_panel(panel_node: TopologyNode, panel_ports: Sequence[TopologyNode], pos: QPointF, height: float):
                    item = (
                        SplitterFrontPanelItem(panel_node, panel_ports, panel_width, 88.9)
                        if panel_node.asset_type == "fibre_splitter"
                        else SwitchFrontPanelItem(panel_node, panel_ports, panel_width, height)
                    )
                    item.setPos(pos)
                    item.setZValue(1.0)
                ...
                if node.asset_type == "network_switch" and stack_members > 1:
                    ports_by_member: Dict[int, List[TopologyNode]] = defaultdict(list)
                    for port_node in port_nodes:
                        member = min(stack_members, max(1, self._port_stack_member_number(port_node)))
                        ports_by_member[member].append(port_node)
                    member_units = max(
                        1,
                        _int(
                            node.asset.get("switch_rack_unit_allowance"),
                    ...
                    panel_height = max(88.9, member_units * 88.9)
                    panel_gap = 34.0
                    base = positions[switch_id]
                    first_item = None
                    for member in range(1, stack_members + 1):
                        member_ports = ports_by_member.get(member, [])
                        member_instance = dict(node.instance)
                        member_instance["logical_stack"] = False
                        member_instance["stack_member_count"] = 1
                        member_node = replace(
                            node,
                            name=f"{node.name} - Switch {member}",
                            instance=member_instance,
                            port_capacity=len(member_ports),
                            ports_used=sum(1 for port in member_ports if bool(port.details.get("occupied"))),
                        )
                        item = add_panel(
                            member_node,
                            member_ports,
                            QPointF(base.x(), base.y() + (member - 1) * (panel_height + panel_gap)),
                            panel_height,
                        )
                        if first_item is None:
                            first_item = item
                    if first_item is not None:
                        self.node_items[switch_id] = first_item
                else:
                    units = max(1, self._node_rack_units(switch_id))
                    panel_height = max(88.9, units * 88.9)
                    item = add_panel(node, port_nodes, positions[switch_id], panel_height)
            ...
            for node_id in visible_ids:
                ...
                if self.rack_focus is not None:
                    ...
                    if shared_group and shared_capacity > 1:
                        ...
                        equipment_height = half_units * (44.45 / 2.0) - 4.0
                        display_units = units
    ...
    def _card_rect(self, node_id: str, positions: Dict[str, QPointF]) -> QRectF:
        ...
        if self.rack_focus is not None and self.switch_port_focus is None:
            ...
            height = max(1, self._node_rack_half_units(node_id)) * (44.45 / 2.0) - 4.0
    ...
    def _room_group_key(self, node_id: str) -> Tuple[int, str]:
        node = self._node_for(node_id)
        if node is None or node.pseudo:
            return (0, "")
        location = node.location_name
        ...
        if not location and not rack:
            return (0, "")
        if self._rack_names_for_location(node.floor, location):
            return (node.floor, location)
        return (0, "")
    ...
    @staticmethod
    def _node_rack_start_half_u(node: TopologyNode) -> int:
        explicit = max(0, _int(node.instance.get("rack_start_half_u")))
        if explicit:
            return explicit
        return (max(1, _int(node.instance.get("rack_start_u"), 1)) - 1) * 2 + 1

    def _node_rack_half_units(self, node_id: str) -> int:
        node = self._node_for(node_id)
        if node is None:
            return 0
        member_slots = [
            max(1, _int(value, 1))
            for value in node.instance.get("stack_member_start_half_us", [])
        ]
        if len(member_slots) > 1:
            return max(member_slots) + 1 - min(member_slots) + 1
        explicit = max(
            0,
            _int(node.instance.get("rack_height_half_u")),
            _int(node.asset.get("rack_height_half_u")),
        )
        if explicit:
            return explicit
        return max(0, self._node_rack_units(node_id) * 2)
    ...
    def _add_location_groups(self, visible_ids: Sequence[str], positions: Dict[str, QPointF]) -> None:
        ...
        for key, node_ids in rack_groups.items():
            floor, location, _rack = key
            grouped[(floor, location)][key] = node_ids

        for (_floor, location), racks in grouped.items():
            rect: Optional[QRectF] = None
            bus_nodes: List[str] = []
            for node_ids in racks.values():
                for node_id in node_ids:
                    if node_id not in positions:
                        continue
                    card_rect = self._card_rect(node_id, positions)
                    rect = card_rect if rect is None else rect.united(card_rect)
                    if node_id in getattr(self, "_location_bus_by_node", {}):
            ...
            if bus_nodes:
                ...
                if any(node_id in failover_nodes for node_id in bus_nodes):
                    ...
                    fail_pen = QPen(self._secondary_link_colour(), 2.0, Qt.DashLine)
    ...
    @staticmethod
    def _secondary_link_colour() -> QColor:
        return QColor("#e84cff")
    ...
    def _edge_has_parallel_secondary(
        self,
        ...
        if edge is None or _text(edge.medium).lower() != "fibre":
            return False
        target_node = self._node_for(target_id) if target_id else None
        if target_node is not None and target_node.connection_count > 1:
            return True
        connection = edge.connection if isinstance(edge.connection, dict) else {}
        member_count = max(
            _int(connection.get("link_aggregation_member_count"), 0),
            _int(connection.get("parallel_link_count"), 0),
            _int(connection.get("uplink_count"), 0),
        )
        if member_count > 1:
            return True
        speed = max(0.0, _float(connection.get("link_speed_mbps")))
        aggregate_speed = max(0.0, _float(connection.get("aggregate_link_speed_mbps")))
        return bool(speed and aggregate_speed > speed * 1.1)
    ...
    def _edge_colour(self, edge: Optional[TopologyEdge], medium: str = "") -> QColor:
        ...
        if self._edge_is_failover(edge):
            return self._secondary_link_colour()
    ...
    def _failover_fibre_lane_x(
        ...
        lane_offset = lane_index * 96.0
        preferred = (
            self._failover_bus_column_x
            if self._failover_bus_column_x is not None
            else primary_lane - 220.0
        ) - lane_offset
        return min(
            preferred,
            primary_lane - 260.0 - lane_offset,
            bus_left - required_span - 304.0 - lane_offset,
        )
    ...
    def _add_link_rail(self, source_id: str, child_ids: Sequence[str], positions: Dict[str, QPointF]) -> None:
        ...
        # Core, distribution and OLT outputs represent separate physical
        # uplink/PON ports. Draw one routed link per child rather than merging
        # them into a shared rail, which makes access uplinks look like they
        # run across neighbouring access switches.
        if source_node is not None and (
            source_node.asset_type == "optical_line_terminal"
            or source_node.role.startswith("olt_")
            or "core" in source_node.role
            or "aggregation" in source_node.role
        ...
        for _location, group_child_ids in grouped.items():
            ...
            for child_id, end, edge in endpoints:
                ...
                colour = self._edge_colour(edge, medium) if edge else (self._secondary_link_colour() if standby else self._link_colour(medium))
    ...
    def _add_shared_failover_trunk(
        ...
        colour = self._secondary_link_colour()
        ...
        for target_id, target_point, edge in target_rows:
            ...
            if branch_end_x - trunk_x < required:
                ...
                label_point = QPointF((label_lane_x + branch_end_x) / 2.0, target_point.y() + 10.0)
            else:
                branch = QPainterPath(QPointF(trunk_x, target_point.y()))
                branch.lineTo(target_point)
                label_point = QPointF((trunk_x + branch_end_x) / 2.0, target_point.y() + 10.0)
    ...
    def _add_cross_link_rail(
        ...
        if len(bus_endpoints) == len(endpoints):
            ...
            trunk_item.setPen(QPen(self._secondary_link_colour(), 2.0, Qt.DashLine))
            trunk_item.setZValue(-0.82)
            self.scene.addItem(trunk_item)
            for target_id, end, edge in endpoints:
                self._add_bus_drop(target_id, positions, self._secondary_link_colour(), failover=True, z_value=-0.8, edge=edge)
                if self.show_link_labels_check.isChecked():
                    self._add_link_label(edge.label, self._secondary_link_colour(), QPointF((entry_x + end.x()) / 2.0, end.y() + 10.0), edge)
            return
        ...
        self._add_vertical_link_rail(start, endpoints, self._secondary_link_colour(), z_value=-0.82, cross_link=True, rail_x=rail_x)
    ...
    def _incoming_fibre_connection_x(
        ...
        if edge is None or _text(edge.medium).lower() != "fibre":
            return centre_x
        incoming = self._incoming_fibre_edges_for_node(node_id, edge)
        if self._edge_has_parallel_secondary(edge, node_id):
            edge_margin = 36.0
            x = centre_x + (32.0 if failover else -32.0)
            return max(pos.x() + edge_margin, min(pos.x() + self.CARD_W - edge_margin, x))
        selected_index, count = self._incoming_fibre_slot_index(node_id, edge)
        if count <= 1:
            return centre_x
    ...
    def _add_link(
        ...
        if target_bus is not None and not client_link:
            ...
            if target_right_of_source and failover_style:
                ...
                lane_offset = (
                    120.0
                    + self._failover_lane_index(edge, source_id, target_id) * 96.0
            ...
            if not failover_style and self._edge_has_parallel_secondary(edge, target_id):
                secondary_bus = self._bus_for_node(target_id, failover=True)
                if secondary_bus is not None:
                    secondary_anchor, secondary_left, secondary_right = secondary_bus
                    secondary_entry_x = (
                        self._failover_fibre_lane_x(
                            secondary_left, required_span, edge, source_id, target_id
                        )
                        if target_right_of_source
                        else secondary_right + required_span + 154.0
                    )
                    secondary_bus_entry_x = secondary_left if target_right_of_source else secondary_right
                    secondary_start = QPointF(
                        source_pos.x() + self.CARD_W,
                        self._outgoing_fibre_connection_y(
                            source_id, positions, edge, failover=True
                        ),
                    )
                    secondary_path = QPainterPath(secondary_start)
                    secondary_path.lineTo(QPointF(secondary_entry_x, secondary_start.y()))
                    secondary_path.lineTo(QPointF(secondary_entry_x, secondary_anchor.y()))
                    secondary_path.lineTo(QPointF(secondary_bus_entry_x, secondary_anchor.y()))
                    secondary_item = InteractiveLinkItem(
                        secondary_path,
                        edge.edge_id if edge else "",
                        self._edge_context_menu,
                    )
                    secondary_item.setPen(QPen(self._secondary_link_colour(), 2.0, Qt.DashLine))
                    secondary_item.setZValue(-0.8)
                    self.scene.addItem(secondary_item)
                    self._add_bus_drop(
                        target_id,
                        positions,
                        self._secondary_link_colour(),
                        failover=True,
                        z_value=-0.8,
                        edge=edge,
                    )
        ...
        medium = edge.medium if edge else ("virtual" if client_link else "copper")
        colour = self._edge_colour(edge, medium) if edge is not None else (self._secondary_link_colour() if failover_style else self._link_colour(medium))
        traced = bool(edge and (edge.edge_id in self.trace_connection_ids or _text(edge.connection.get("parent_logical_connection_id")) in self.trace_connection_ids))
        pen = QPen(colour, 3.4 if traced else (2.0 if not client_link else 1.3))
    ...
    def _card_activated(self, node_id: str) -> None:
        ...
        if item is not None and not item.isSelected() and not modified_selection:
            self.scene.clearSelection()
            item.setSelected(True)
    ...
    def _select_front_panel_port(self, port_node_id: str) -> None:
        parent_id = ""
        node = self._node_for(port_node_id)
        if node is not None:
            parent_id = _text(node.details.get("parent_instance_id"))
        for item_id, item in self.node_items.items():
            setter = getattr(item, "set_selected_port", None)
            if callable(setter):
                setter(port_node_id if item_id == parent_id else "")
    ...
    def _open_selected_room_view(self) -> None:
        node_id = _text(self.room_view_button.property("node_id"))
        key = self._room_group_key(node_id)
        if key == (0, ""):
            if self.rack_focus is not None:
                floor, location, _rack = self.rack_focus
                key = (floor, location)
            else:
                return
        floor, location = key
        dialog = CommsRoomIsometricDialog(self, floor, location, self)
        dialog.setAttribute(Qt.WA_DeleteOnClose, True)
        dialog.destroyed.connect(lambda _obj=None, dlg=dialog: self._room_view_dialogs.remove(dlg) if dlg in self._room_view_dialogs else None)
        self._room_view_dialogs.append(dialog)
        dialog.show()
        dialog.raise_()
    ...
    def _rack_empty_space_double_clicked(self, scene_position: QPointF) -> None:
        ...
        if not (rack_top <= scene_position.y() < rack_top + rack_height):
            return
        half_row_from_top = int(
            (scene_position.y() - rack_top) // (unit_pitch / 2.0)
        )
        rack_half_u = capacity * 2 - half_row_from_top
        rack_u = int(math.ceil(rack_half_u / 2.0))
        for node_id, node in self.model.nodes.items():
            ...
            if (
                ...
                continue
            start_half_u = self._node_rack_start_half_u(node)
            half_units = max(0, self._node_rack_half_units(node_id))
            if (
                half_units > 0
                and start_half_u
                <= rack_half_u
                <= start_half_u + half_units - 1
            ):
                self.status_label.setText(
    ...
    def focus_location(self, location_name: str) -> bool:
        """Reveal and centre the topology devices installed at a location."""
        ...
        candidate_ids = [
            ...
            for node_id, node in self.model.nodes.items()
            if not node.pseudo
            and _text(node.location_name) == location_name
            and self._is_active_topology_device(node_id)
        ]
        if not candidate_ids:
            self.status_label.setText(
                f"{location_name} has no devices shown in the topology map."
            )
            return False
        ...
        # Dense topology branches may be collapsed by default. Open only the
        # ancestor paths needed to reveal this location, leaving other branches
        # and the user's existing expansion choices untouched.
        for node_id in candidate_ids:
            parent_id = self.model.parent.get(node_id, "")
            guard = 0
            while parent_id and guard <= len(self.model.nodes):
                self.explicit_expanded.add(parent_id)
                self.explicit_collapsed.discard(parent_id)
                parent_id = self.model.parent.get(parent_id, "")
                guard += 1

        self.search_edit.setText(location_name)
        self.rebuild_scene(fit=False)
        visible_ids = [
            node_id for node_id in candidate_ids if node_id in self.node_items
        ]
        if not visible_ids:
            self.status_label.setText(
                f"{location_name} has no devices shown in the topology map."
        ...
        self.view.focus_on_item(self.node_items[first_id])
        self._show_node_details(first_id)
        self.status_label.setText(
            f"Found {len(visible_ids)} topology device"
            f"{'s' if len(visible_ids) != 1 else ''} at {location_name}."
        )
        return True
