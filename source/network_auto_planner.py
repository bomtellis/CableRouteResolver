...
def _traffic_concurrency_factors(record: dict) -> Tuple[float, float]:
    """Return directional busy-hour concurrency as bounded multipliers.

    Factors belong to traffic-producing endpoint assets. Generated instances,
    assignments and connections already carry diversified demand, so callers
    should apply these multipliers only while creating endpoint demand.
    """
    ...
    return (
        min(
            1.0,
            max(0.0, _float(record.get("north_south_concurrency_factor"), 1.0)),
        ),
        min(
            1.0,
            max(0.0, _float(record.get("east_west_concurrency_factor"), 1.0)),
        ),
    )
...
class RoutingGraph:
    ...
    def __init__(self, data: dict):
        ...
        self._floor_trees: Dict[
            Tuple[str, int], Tuple[Dict[str, float], Dict[str, str]]
        ] = {}
        self._build()
    ...
    def _build(self) -> None:
        ...
        for point_name, point in list(self.points.items()):
            if _text(point.get("kind")).lower() in {"corridor_node", "transition_node"}:
                continue
            # Links to another room/device do not make this point part of the
            # corridor graph.  Generated adjacent DERs, for example, form a
            # small equipment-room component that still needs one surveyed
            # spur into the nearest corridor node.
            if any(
                _text(self.points.get(neighbour, {}).get("kind")).lower()
                in {"corridor_node", "transition_node"}
                for neighbour, _weight in self.graph.get(point_name, [])
            ):
                continue
    ...
    def route_on_floor(
        self, source: str, destination: str, floor: int
    ) -> Tuple[float, List[str]]:
        """Route without using nodes or transitions on another floor."""
        source = _text(source)
        destination = _text(destination)
        floor = _int(floor)
        if not source or not destination:
            return 0.0, []
        if any(
            _int(self.points.get(name, {}).get("floor")) != floor
            for name in (source, destination)
        ):
            raise NetworkPlanningError(
                f"{source} and {destination} are not both on floor {floor}.",
                code="same_floor_assignment",
                details={
                    "from_name": source,
                    "to_name": destination,
        ...
        key = (source, floor)
        if key not in self._floor_trees:
            allowed = {
                name
                for name, point in self.points.items()
                if _int(point.get("floor")) == floor
            }
            floor_graph = {
                name: [
                    (neighbour, weight)
                    for neighbour, weight in self.graph.get(name, [])
                    if neighbour in allowed
                ]
                for name in allowed
            }
            self._floor_trees[key] = _dijkstra_tree(floor_graph, source)
        distances, parents = self._floor_trees[key]
        if destination not in distances:
            raise NetworkPlanningError(
                f"No floor-{floor} routing path connects {source} to {destination}.",
                code="same_floor_route_unavailable",
                details={
                    "from_name": source,
                    "to_name": destination,
                    "required_floor": floor,
...
def synchronise_network_connection_routes(
    data: dict,
    connection_ids: Optional[Iterable[str]] = None,
    *,
    force: bool = False,
) -> dict:
    """Route logical network links and endpoint assignments over the route graph.

    Manual network links historically kept an empty ``route_path`` and were
    therefore rendered as straight device-to-device lines.  This uses the same
    ``RoutingGraph`` as the automatic planner.  Full-project synchronisation
    also repairs endpoint assignments; hidden patch leads and physical fibre
    segments retain their own routes.
    """

    requested_ids = {
        _text(value) for value in (connection_ids or []) if _text(value)
    }
    ...
    def anchor(instance: dict) -> str:
        return _text(instance.get("route_anchor")) or _text(
            instance.get("location_name")
        )

    def route_is_valid(current_path: Sequence[str], source: str, target: str) -> bool:
        if source == target:
            return list(current_path) == [source]
        return (
            bool(current_path)
            and current_path[0] == source
            and current_path[-1] == target
            and all(
                any(neighbour == b for neighbour, _weight in graph.graph.get(a, []))
    ...
    for connection in data.get("network_connections", []):
        ...
        if requested_ids and connection_id not in requested_ids:
            continue
        ...
        if source_anchor not in graph.points or target_anchor not in graph.points:
            skipped.append(connection_id)
            continue
        ...
        path_is_valid = route_is_valid(current_path, source_anchor, target_anchor)
        if path_is_valid and not force:
            continue

        length_m, route_path = graph.route(source_anchor, target_anchor)
        connection["route_path"] = list(route_path)
        connection["length_m"] = round(max(0.0, float(length_m)), 3)
        connection["route_source"] = source_anchor
        connection["route_destination"] = target_anchor
        updated.append(connection_id)

    # A scoped call is used when one manual logical link changes.  Endpoint
    # assignments are repaired only during full project/open synchronisation.
    if not requested_ids:
        for assignment in data.get("network_endpoint_assignments", []):
            if not isinstance(assignment, dict):
                continue
            assignment_id = _text(assignment.get("id"))
            source = instances.get(_text(assignment.get("network_instance_id")))
            source_anchor = anchor(source) if source else ""
            endpoint_name = _text(assignment.get("endpoint_name"))
            if (
                not source_anchor
                or source_anchor not in graph.points
                or endpoint_name not in graph.points
            ):
                skipped_assignments.append(assignment_id)
                continue

            current_path = [
                _text(value)
                for value in assignment.get("route_path", [])
                if _text(value)
            ]
            if route_is_valid(current_path, source_anchor, endpoint_name) and not force:
                continue

            length_m, route_path = graph.route(source_anchor, endpoint_name)
            extension_m = max(
                0.0, _float(graph.points[endpoint_name].get("extension_distance_m"))
            )
            assignment["route_path"] = list(route_path)
            assignment["copper_length_m"] = round(
                max(0.0, float(length_m) + extension_m), 3
            )
            updated_assignments.append(assignment_id)

    return {
        "updated_connection_ids": updated,
        "skipped_connection_ids": skipped,
        "updated_assignment_ids": updated_assignments,
        "skipped_assignment_ids": skipped_assignments,
    }


def align_auto_edge_routers_to_cores(data: dict) -> dict:
    """Move legacy auto edge routers beside their connected MER cores.

    Older plans selected edge locations from every comms-room candidate before
    the core association was considered.  Repair only planner-owned routers;
    manually placed edge equipment remains exactly where the user put it.
    """
    ...
    core_by_edge: Dict[str, List[dict]] = defaultdict(list)
    for connection in data.get("network_connections", []):
        if (
        ...
        if not left or not right:
            continue
        left_role = _text(left.get("design_role")).lower()
        right_role = _text(right.get("design_role")).lower()
        if left_role == "edge_router" and right_role == "core_switch":
            core_by_edge[_text(left.get("id"))].append(right)
        elif right_role == "edge_router" and left_role == "core_switch":
            core_by_edge[_text(right.get("id"))].append(left)

    rack_capacity = max(
        1, _int(data.get("network_settings", {}).get("default_rack_size_u"), 42)
    )

    def rack_units(instance: dict) -> int:
        ...
        return max(1, _rack_units_for_asset(asset, members))

    def free_start(edge_id: str, location_name: str, rack_name: str, units: int) -> int:
        occupied: List[Tuple[int, int]] = []
        for other_id, other in instances.items():
            if (
                other_id == edge_id
                or _text(other.get("location_name")) != location_name
                or _text(other.get("rack_name")) != rack_name
            ):
                continue
            start = max(1, _int(other.get("rack_start_u"), 1))
            occupied.append((start, start + rack_units(other) - 1))
        for start in range(1, rack_capacity - units + 2):
            end = start + units - 1
    ...
    updated: List[str] = []
    for edge_id, cores in sorted(core_by_edge.items()):
        edge = instances.get(edge_id, {})
        if not bool(edge.get("auto_generated")) or not cores:
            continue
        preferred_core_id = _text(edge.get("associated_core_instance_id"))
        core = next(
            (row for row in cores if _text(row.get("id")) == preferred_core_id),
            sorted(cores, key=lambda row: _text(row.get("id")))[0],
        )
        location_name = _text(core.get("location_name"))
        location = locations.get(location_name)
        rack_name = _text(core.get("target_rack_name") or core.get("rack_name"))
        if location is None or not rack_name:
            continue
        changed = (
            _text(edge.get("location_name")) != location_name
            or _text(edge.get("route_anchor")) != (
                _text(core.get("route_anchor")) or location_name
            )
            or _text(edge.get("rack_name")) != rack_name
        )
        if not changed:
            continue
        units = rack_units(edge)
        start_u = free_start(edge_id, location_name, rack_name, units)
        if not start_u:
            # Keep it in the MER but use an adjacent rack if the core rack is
            # full; the normal rack repacker will consolidate it on the next run.
            rack_name = f"{rack_name}-EDGE"
            start_u = 1
        edge.update(
            {
                ...
                "route_anchor": _text(core.get("route_anchor")) or location_name,
                "rack_name": rack_name,
                "target_rack_name": rack_name,
                "rack_start_u": start_u,
                "associated_core_instance_id": _text(core.get("id")),
            }
        )
        updated.append(edge_id)
    return {"updated_instance_ids": updated}
...
def build_endpoint_demands(data: dict) -> Tuple[List[EndpointDemand], List[str]]:
    ...
    room_names = {
        ...
        in {"comms_room", "distributed_equipment_room", "mer"}
    ...
    for item in data.get("connections", []):
        ...
        # Only Routing-tab equipment-room connections define endpoint
        # ownership. Generated switch-to-endpoint connections must not replace
        # that source when a completed network is regenerated.
        if from_name in room_names and to_name:
            existing_sources[to_name] = from_name
        elif to_name in room_names and from_name:
            existing_sources[from_name] = to_name
    ...
    for instance_id, instance in sorted(network_instances.items()):
        ...
        for field_name in (
            ...
            "expected_north_south_bandwidth_mbps",
            "expected_east_west_bandwidth_mbps",
            "north_south_concurrency_factor",
            "east_west_concurrency_factor",
        ):
        ...
        north_south_factor, east_west_factor = _traffic_concurrency_factors(
            source_traffic
        )
        north_south *= north_south_factor
        east_west *= east_west_factor
        bandwidth = north_south + east_west
        packets = max(
            0.0,
            _float(
                instance.get(
...
def _minimum_asset_mix(
    ...
    label: str,
    *,
    endpoint_ports_only: bool = False,
) -> List[dict]:
    """Return a minimum-device-count capacity mix using a Pareto frontier."""

    def port_capacity(asset: dict) -> int:
        if endpoint_ports_only:
            return len(_asset_endpoint_port_slots(asset))
        return max(0, _int(asset.get("number_of_ports")))

    candidates = [item for item in candidates if port_capacity(item) > 0]
    if not candidates:
        raise NetworkPlanningError(
    ...
    max_ports = max(port_capacity(item) for item in candidates)
    max_poe = max(
    ...
    for device_count in range(1, upper + 1):
        ...
        for (ports, poe), (indices, preference, power, rack) in frontier.items():
            for candidate_index, asset in enumerate(candidates):
                next_ports = min(port_cap, ports + port_capacity(asset))
                next_poe = min(
...
def _asset_endpoint_port_slots(asset: dict) -> List[dict]:
    ...
    for row in endpoint_rows:
        ...
        prefix = _text(row.get("name_prefix")) or {
            ...
            "sfp": "SFP", "sfp+": "SFP+", "sfp28": "SFP28", "sfp56": "SFP56",
...
def _pack_ports_to_devices(
    ...
    spare_fraction: float,
    *,
    reserve_ports_per_device: bool = False,
    fill_device_groups: int = 0,
) -> List[List[PortDemand]]:
    # Spare capacity is enforced when selecting the aggregate device mix.  The
    # normal physical allocation balances endpoints across that mix. DER callers
    # can instead fill each two-switch room as a group before using the next one.
    def allocation_slots(asset: dict) -> List[dict]:
        slots = list(_asset_endpoint_port_slots(asset))
        if not reserve_ports_per_device or spare_fraction <= 0.0:
            return slots
        usable_count = max(
            0,
            int(
                math.floor(
                    len(slots) / (1.0 + max(0.0, float(spare_fraction)))
                )
            ),
        )
        slots.sort(
            key=lambda row: max(
                normalise_port_speeds(row.get("speeds")) or [0]
            ),
            reverse=True,
        )
        return slots[:usable_count]

    devices = [
        {
            "slots": allocation_slots(asset),
        ...
        for device_index, device in enumerate(devices):
            ...
            for slot_index, slot in enumerate(device["slots"]):
                ...
                choices.append(
                    (
                        (
                            device_index // max(1, fill_device_groups)
                            if fill_device_groups > 0
                            else 0
                        ),
...
def _clear_previous_auto_design(data: dict) -> None:
    ...
    def generated_location(item: dict) -> bool:
        ...
        if bool(item.get("auto_network_location")):
            return True
        # Recover legacy planner-owned rows saved before the ownership flag was
        # consistently persisted.  Manual DERs never carry an overflow parent.
        return bool(
            _text(item.get("overflow_parent_location"))
            and _text(item.get("kind")).lower()
            == "distributed_equipment_room"
        ) or _text(item.get("name")).startswith("AUTO-POLAN-")

    removed_location_names = {
        _text(item.get("name"))
        for item in data.get("locations", [])
    ...
    # Resolution choices for a generated overflow DER belong to that generated
    # row, not to the next planner run.  Retain choices made against manual
    # parent rooms while removing stale generated-location keys.
    overrides = data.setdefault("network_settings", {}).get(
        "auto_planner_resolution_overrides", {}
    )
    if isinstance(overrides, dict) and removed_location_names:
        for key in (
            "access_asset_by_location",
            "minimum_access_switches_by_location",
            "spare_capacity_mode_by_location",
            "additional_der_by_location",
            "additional_der_asset_by_location",
        ):
            values = overrides.get(key)
            if isinstance(values, dict):
                for location_name in removed_location_names:
                    values.pop(location_name, None)
...
class DesignBuilder:
    ...
    def _expanded_ports(self, instance: dict) -> List[dict]:
        ...
        configured_rows = instance.get("port_definitions")
        if not isinstance(configured_rows, list) or not configured_rows:
            configured_rows = asset.get("port_definitions", [])
        rows = [row for row in configured_rows if isinstance(row, dict) and _int(row.get("port_count")) > 0]
        result: List[dict] = []
        counters: Dict[str, int] = defaultdict(int)
        for row in rows:
    ...
    def connection_bundle_capabilities(
        ...
        free_left_port_count = intended_free_count(
            from_instance, left_rows, requested_from, "uplink"
        )
        free_right_port_count = intended_free_count(
            to_instance, right_rows, requested_to, "input"
        )
        ...
        return {
            ...
            "free_left_port_count": free_left_port_count,
            "free_right_port_count": free_right_port_count,
                ...
                access_alternatives.append(
                    {
                        ...
                        **capability,
                        "meets_required": capability["capacity_mbps"] > 0
                        and capability["capacity_mbps"] + 1e-9
        ...
        if not choices:
            ...
            raise NetworkPlanningError(
                ...
                details={
                    ...
                    "capabilities": capabilities,
                    "free_from_port_count": max(
                        0, _int(bundle_info.get("free_left_port_count"))
                    ),
                    "free_to_port_count": max(
                        0, _int(bundle_info.get("free_right_port_count"))
                    ),
    ...
    def add_patch_lead(self, *, connection_id: str = "", assignment_id: str = "", instance: dict, port: str, medium: str, peer_instance_id: str = "", peer_port: str = "", endpoint_name: str = "", preferred_use: str = "", length_m: Optional[float] = None) -> dict:
        definition = self._port_definition(instance, port, medium, preferred_use)
        default_length = (
            max(0.0, float(length_m))
            if length_m is not None
            else (2.0 if medium in {"copper", "fibre"} else 0.0)
        )
        lead = {
    ...
    def add_der_overflow_location(
        ...
        name = base_name
        suffix = 2
        while name in self.location_names:
            name = f"{base_name}-{suffix}"
            suffix += 1
        self.location_names.add(name)

        # Keep the adjacent node close enough to represent the same equipment
        # area, but far enough from its parent and other graph nodes to remain
        # independently visible/selectable in the plan view.
        spacing_m = 3.0
        parent_x = _float(parent.get("x"))
        parent_y = _float(parent.get("y"))
        parent_floor = _int(parent.get("floor"))
        candidate_offsets = []
        for ring in range(1, max(3, overflow_index + 1)):
            distance = spacing_m * ring
        ...
        x, y = parent_x + spacing_m * (overflow_index - 1), parent_y
        for dx, dy in candidate_offsets:
            candidate_x = parent_x + dx
            candidate_y = parent_y + dy
            if all(
                _int(point.get("floor")) != parent_floor
                or math.hypot(
                    candidate_x - _float(point.get("x")),
                    candidate_y - _float(point.get("y")),
                )
                >= spacing_m * 0.8
        ...
        location = {
            **deepcopy(parent),
            ...
            "max_network_cabinets": 1,
            "der_switch_capacity": 2,
            "auto_network_location": True,
            "overflow_parent_location": parent_name,
            "overflow_index": overflow_index,
        }
        self.locations.append(location)
        self.graph._add_point(name, location, "distributed_equipment_room")

        # Every overflow DER is anchored directly to the first DER.  This
        # mirrors the original DER's graph reachability and avoids a fragile
        # chain where removing one intermediate generated DER disconnects all
        # later rooms.
        previous_name = parent_name
        previous = self.graph.points.get(parent_name)
        current = self.graph.points.get(name)
        if previous and current:
            weight = _distance(previous, current, self.graph.floor_height_m)
            self.graph.graph[previous["name"]].append((name, weight))
            self.graph.graph[name].append((previous["name"], weight))
            self.graph._trees.clear()
        self.data.setdefault("corridors", {}).setdefault("edges", []).append(
            {
                "from": previous_name if previous_name in self.graph.points else parent_name,
                "to": name,
                "auto_network_edge": True,
                "edge_role": "der_overflow_adjacency",
            }
        )
        self.warnings.append(
            f"{parent_name} requires more than two access switches; created adjacent "
            f"distributed equipment room node {name}."
        )
        return location
...
def _is_distributed_equipment_room(location: dict) -> bool:
    """Recognise explicit DER locations and legacy DER-prefixed rooms."""
...
def _asset_layer_downlink_port_capacity(asset: dict) -> int:
    """Return ports the runtime selector can use for downstream layer links.

    When a switch declares dedicated downlink ports, connection allocation
    intentionally prefers only those ports.  Layer sizing must use the same
    inventory instead of also counting dedicated uplinks, otherwise a core can
    be selected with apparent spare capacity that cannot terminate another
    access link.
    """

    # Uplink/downlink are logical roles on modern switching interfaces. Count
    # both pools for layer sizing; the allocator will record when a declared
    # role is reconfigured for the selected connection.
    downlinks = sum(
        max(0, _int(row.get("port_count")))
        for row in asset.get("port_definitions", [])
        if isinstance(row, dict)
        and _text(row.get("port_use")).lower() in {"downlink", "uplink"}
        and _text(row.get("port_type")).lower()
        in PLUGGABLE_OPTIC_PORT_TYPES | {"lc", "sc", "mpo", "pon"}
    )
    return downlinks if downlinks > 0 else _asset_layer_port_capacity(asset)


def _layer_capacity_candidates(candidates: Sequence[dict]) -> List[dict]:
    return [
        {**asset, "number_of_ports": _asset_layer_downlink_port_capacity(asset)}
        for asset in candidates
        if _asset_layer_downlink_port_capacity(asset) > 0
    ]


def _representative_layer_targets(targets: Sequence[dict]) -> List[dict]:
    """Keep the highest-demand instance of each installed port inventory."""
    representatives = {}
    for target in targets:
        key = (
            _text(target.get("asset_id")),
            tuple(
                (
                    _text(row.get("port_type")),
                    _text(row.get("port_use")),
                    _int(row.get("port_count")),
                    tuple(normalise_port_speeds(row.get("supported_speeds_mbps"))),
                )
                for row in target.get("port_definitions", []) or []
                if isinstance(row, dict)
            ),
        )
        existing = representatives.get(key)
        if existing is None or _float(
            target.get("expected_bandwidth_mbps")
        ) > _float(existing.get("expected_bandwidth_mbps")):
            representatives[key] = target
    return list(representatives.values())


def _modular_layer_configuration_candidates(
    builder: DesignBuilder, candidates: Sequence[dict], targets: Sequence[dict]
) -> List[dict]:
    """Expand a modular chassis into viable installed line-card populations.

    Access/aggregation demand is already known when this is called. The later
    compatibility and capacity passes therefore select a card population from
    real downstream port speeds instead of accepting the library's example
    chassis configuration as fixed hardware.
    """

    result = []
    for asset in candidates:
        if not bool(asset.get("modular_chassis", False)):
            result.append(asset)
            continue
        modules = [
            dict(row)
            for row in asset.get("chassis_modules", []) or []
            if isinstance(row, dict)
        ]
        model_identity = " ".join(
            (
        ...
        if not (
            ...
            # Other modular platforms retain their declared module inventory
            # until they expose an editable line-card catalogue in the schema.
            result.append(asset)
            continue
        supervisor_rows = [
            row for row in modules if _text(row.get("module_type")) == "supervisor"
        ]
        line_slots = (1, 2, 5, 6)
        demanded_speeds = {40000, 100000}
        for target in _representative_layer_targets(targets):
            target_asset = builder._asset_for_instance(target)
            for row in target.get("port_definitions", []) or target_asset.get(
                "port_definitions", []
            ) or []:
                if isinstance(row, dict) and (
                    _text(row.get("medium")).lower() == "fibre"
                    or _text(row.get("port_type")).lower()
                    in PLUGGABLE_OPTIC_PORT_TYPES | {"lc", "sc", "mpo"}
                ):
                    demanded_speeds.update(
                        normalise_port_speeds(row.get("supported_speeds_mbps"))
                    )
        card_vectors = {}
        for model, groups in CATALYST_9600_LINE_CARDS.items():
            card_vectors[model] = tuple(
                sum(count for _port_type, count, _use, speeds in groups if speed in speeds)
                for speed in sorted(demanded_speeds)
            )
        models = []
        for model, vector in card_vectors.items():
            dominated = any(
                other != model
                and all(left <= right for left, right in zip(vector, other_vector))
                and any(left < right for left, right in zip(vector, other_vector))
                for other, other_vector in card_vectors.items()
            )
            if not dominated and any(vector):
                models.append(model)
        models.sort()
        for card_models in combinations_with_replacement(models, len(line_slots)):
            configured_modules = [dict(row) for row in supervisor_rows]
            configured_modules.extend(
                {
                    "slot": slot,
                    "module_type": "line_card",
                    "model": model,
                }
                for slot, model in zip(line_slots, card_models)
            )
            configured_modules.sort(key=lambda row: _int(row.get("slot")))
            port_definitions = catalyst_9600_port_definitions(configured_modules)
            port_count = sum(
                max(0, _int(row.get("port_count")))
                for row in port_definitions
            )
            if port_count <= 0:
                continue
            result.append(
                {
                    **asset,
                    "number_of_ports": port_count,
                    "port_definitions": port_definitions,
                    "chassis_modules": configured_modules,
                    "_planner_chassis_modules": configured_modules,
                    "_planner_port_definitions": port_definitions,
                    "_planner_modular_configuration": True,
                    "_planner_line_card_models": list(card_models),
                }
            )
    return result
...
def _candidate_bundle_members_for_target(
    ...
    fake_source = {
        ...
        "network_layer": "core",
        "design_role": "core_switch",
        "port_definitions": deepcopy(
            source_asset.get("_planner_port_definitions")
            or source_asset.get("port_definitions", [])
        ),
        "chassis_modules": deepcopy(
            source_asset.get("_planner_chassis_modules")
            or source_asset.get("chassis_modules", [])
        ),
    }
    info = builder.connection_bundle_capabilities(
        target,
...
def _compatible_layer_candidates(
    ...
    representative_targets = _representative_layer_targets(targets)
    rows = [
        asset
        for asset in candidates
        if all(
...
def _speed_compatible_layer_capacity_candidates(
    builder: DesignBuilder,
    candidates: Sequence[dict],
    targets: Sequence[dict],
) -> List[dict]:
    """Size layer devices from the ports compatible with every target type.

    A modular core can have many free cages while its 10 Gb/s subset is already
    exhausted. Using the total cage count made the sizing pass select two cores
    and the later allocator fail part-way through the access layer. The
    conservative common capacity below keeps sizing and allocation aligned.
    """

    result = []
    representative_targets = _representative_layer_targets(targets)
    for asset in candidates:
        total_capacity = _asset_layer_downlink_port_capacity(asset)
        if total_capacity <= 0:
            continue
        fake_source = {
            "id": f"__LAYER-CAPACITY-{_text(asset.get('id'))}",
            "asset_id": _text(asset.get("id")),
            "network_layer": "core",
            "design_role": "core_switch",
            "port_definitions": deepcopy(
                asset.get("_planner_port_definitions")
                or asset.get("port_definitions", [])
            ),
            "chassis_modules": deepcopy(
                asset.get("_planner_chassis_modules")
                or asset.get("chassis_modules", [])
            ),
        }
        target_capacities = []
        for target in representative_targets:
            ...
            for capability in info.get("capabilities", []):
                ...
                max_members = max(0, _int(capability.get("max_members")))
                required_members = (
                    max(1, int(math.ceil(required / float(speed) - 1e-12)))
                    if required > 0.0 and speed > 0
                    else 1
                )
                if speed > 0 and required_members <= max_members:
                    compatible_counts.append(
                        len(capability.get("right_rows", []) or [])
                    )
            if info.get("transparent_pair") is not None:
                compatible_counts.append(1)
            target_capacities.append(max(compatible_counts, default=0))
        compatible_capacity = (
            min(target_capacities) if target_capacities else total_capacity
        )
        if compatible_capacity > 0:
            result.append(
                {
                    **asset,
                    "number_of_ports": min(total_capacity, compatible_capacity),
                    "_planner_total_layer_port_capacity": total_capacity,
                    "_planner_speed_compatible_port_capacity": compatible_capacity,
                }
            )
    # A modular catalogue can yield many card populations with the same usable
    # downstream capacity.  They are equivalent to the capacity optimiser, but
    # retaining every permutation makes its Pareto search grow needlessly.  Keep
    # the most versatile physical population for each useful capacity point.
    compact: List[dict] = []
    modular_by_capacity = {}
    for asset in result:
        if not bool(asset.get("_planner_modular_configuration", False)):
            compact.append(asset)
            continue
        key = (
            _text(asset.get("id")),
            max(0, _int(asset.get("number_of_ports"))),
        )
        existing = modular_by_capacity.get(key)
        score = (
            max(0, _int(asset.get("_planner_total_layer_port_capacity"))),
            tuple(_text(model) for model in asset.get("_planner_line_card_models", [])),
        )
        existing_score = (
            max(0, _int(existing.get("_planner_total_layer_port_capacity"))),
            tuple(
                _text(model)
                for model in existing.get("_planner_line_card_models", [])
            ),
        ) if existing is not None else None
        if existing is None or score > existing_score:
            modular_by_capacity[key] = asset
    compact.extend(modular_by_capacity.values())
    return compact
...
def _build_layer_instances(
    ...
    for index, asset in enumerate(assets):
        ...
        instance = builder.add_instance(
            ...
            modular_chassis_configuration_auto=bool(
                asset.get("_planner_modular_configuration", False)
            ),
            chassis_modules=deepcopy(
                asset.get("_planner_chassis_modules")
                or asset.get("chassis_modules", [])
...
def _build_traditional_layer_topology(
    ...
    if topology_model == "collapsed_core":
        ...
        core_candidates = _modular_layer_configuration_candidates(
            builder, core_candidates, access_switches
        )
        core_candidates = _compatible_layer_candidates(
            builder, core_candidates, access_switches, "core switch"
        )
        core_candidates = _speed_compatible_layer_capacity_candidates(
            builder, core_candidates, access_switches
        )
    ...
    aggregation_candidates = _modular_layer_configuration_candidates(
        builder, aggregation_candidates, access_switches
    )
    aggregation_candidates = _compatible_layer_candidates(
        builder, aggregation_candidates, access_switches, "aggregation switch"
    )
    aggregation_candidates = _speed_compatible_layer_capacity_candidates(
        builder, aggregation_candidates, access_switches
    )
    ...
    core_candidates = _modular_layer_configuration_candidates(
        builder, core_candidates, aggregations
    )
    core_candidates = _compatible_layer_candidates(
...
def _stack_group_plan(
    ...
    *,
    ...
    copper_panel_u: float = 1.0,
    cable_manager_u: float = 0.0,
    ups_reserved_u: int = 5,
    max_switches_per_cabinet: int = 0,
    stacking_enabled: bool = True,
    configured_max_stack_members: int = 8,
) -> List[dict]:
    ...
    panel_u = max(0.5, float(copper_panel_u or 0.5))
    manager_u = max(0.0, float(cable_manager_u or 0.0))
    ...
    while index < len(rows):
        ...
        max_stack_members = (
            ...
            if stacking_enabled and bool(asset.get("supports_stacking"))
            else 1
        )
        max_stack_members = min(
            max_stack_members,
            max(1, int(configured_max_stack_members or 1)),
        )
        max_stack_members = min(
            max_stack_members,
            max(1, rack_size_u // per_member_rack_u),
        )
        if max_switches_per_cabinet > 0:
            max_stack_members = min(
                max_stack_members, max(1, int(max_switches_per_cabinet))
            )
        ...
        if top_of_rack:
            ...
            reserved_bundle_u += int(
                math.ceil(panel_count * (panel_u + manager_u))
            )
...
def _apply_rack_connection_policy(builder: DesignBuilder) -> dict:
    ...
    for (floor, location_name), rack_names in racks_by_location.items():
        policy = location_policies.get(location_name, {})
        cabinet_type = _text(policy.get("cabinet_type")).lower() or "standard"
        maximum_cabinets = max(0, _int(policy.get("max_network_cabinets"), 0))
        if maximum_cabinets and len(rack_names) > maximum_cabinets:
            raise NetworkPlanningError(
                f"{location_name} is limited to {maximum_cabinets} network cabinet(s), "
                f"but the generated design requires {len(rack_names)}.",
                code="location_cabinet_limit",
                details={
                    "location_name": location_name,
                    "maximum_cabinets": maximum_cabinets,
                    "required_cabinets": len(rack_names),
                    "cabinet_type": cabinet_type,
                },
            )
        for row in builder.instances:
            if (
                _int(row.get("floor")) == floor
                and _text(row.get("location_name")) == location_name
                and _text(row.get("rack_name"))
            ):
                row["cabinet_type"] = cabinet_type
        if cabinet_type == "slim_wall":
            for rack_name in rack_names:
                switch_count = sum(
                    max(1, _int(row.get("stack_member_count"), 1))
                    for row in builder.instances
                    if _int(row.get("floor")) == floor
                    and _text(row.get("location_name")) == location_name
                    and _text(row.get("rack_name")) == rack_name
                    and "switch" in _text(row.get("design_role")).lower()
                )
                if switch_count > 2:
                    raise NetworkPlanningError(
                        f"Slim wall cabinet {rack_name} at {location_name} can hold "
                        f"at most two switches, but the generated design places "
                        f"{switch_count} switches in it.",
                        code="slim_wall_cabinet_capacity",
                        details={
                            "location_name": location_name,
                            "rack_name": rack_name,
                            "maximum_switches": 2,
                            "required_switches": switch_count,
                            "cabinet_type": cabinet_type,
                        },
                    )
...
def _traditional_design(
    ...
    comms_rooms = _locations_by_kind(
        builder.data, {"comms_room", "distributed_equipment_room"}
    )
    primary_comms_rooms = [
        room
        for room in comms_rooms
        if not _is_distributed_equipment_room(room)
    ]
    if not comms_rooms:
        raise NetworkPlanningError(
            "Traditional network planning requires at least one comms_room or "
            "distributed_equipment_room location."
        )
    ...
    same_floor_only = bool(
        builder.settings.get("auto_planner_same_floor_only", False)
    )

    def endpoint_route(room: dict, endpoint: EndpointDemand):
        room_name = _text(room.get("name"))
        if same_floor_only:
            return builder.graph.route_on_floor(
                room_name, endpoint.name, endpoint.floor
            )
        return builder.graph.route(room_name, endpoint.name)
    ...
    for _, group in grouped.items():
        ...
        for endpoint in group:
            existing = room_map.get(endpoint.existing_comms_room)
            if existing is not None and (
                not same_floor_only
                or _int(existing.get("floor")) == endpoint.floor
            ):
                weighted_sources[endpoint.existing_comms_room] += endpoint.port_count
        if weighted_sources:
            weighted_primary = {
                name: weight
                for name, weight in weighted_sources.items()
                if not _is_distributed_equipment_room(room_map[name])
            }
            dominant_pool = weighted_primary or weighted_sources
            dominant_name = max(dominant_pool, key=dominant_pool.get)
            ...
            eligible_rooms = primary_comms_rooms or comms_rooms
            if same_floor_only:
                eligible_rooms = [
                    room
                    for room in eligible_rooms
                    if _int(room.get("floor")) == group[0].floor
                ]
            dominant = _nearest_location(centre, eligible_rooms)
        for endpoint in group:
            # A direct Routing-tab room connection is authoritative. Preserve
            # both comms-room and DER assignments made by the graph/zone
            # planner rather than regrouping them onto a departmental room.
            existing_room = room_map.get(_text(endpoint.existing_comms_room))
            if (
                existing_room is not None
                and same_floor_only
                and _int(existing_room.get("floor")) != endpoint.floor
            ):
                raise NetworkPlanningError(
                    f"The existing connection for {endpoint.name} uses "
                    f"{_text(existing_room.get('name'))} on another floor while "
                    "same-floor-only planning is enabled.",
                    code="same_floor_assignment",
                    details={
                        "location_name": _text(existing_room.get("name")),
                        "endpoint_name": endpoint.name,
                        "endpoint_floor": endpoint.floor,
                        "location_floor": _int(existing_room.get("floor")),
                    },
                )
            selected = existing_room or dominant
            if selected is not None:
                route_length, _ = endpoint_route(selected, endpoint)
                route_length += endpoint.extension_distance_m
                if route_length > max_copper_m:
                    if existing_room is not None:
                        builder.warnings.append(
                            f"Preserved the existing Routing-tab connection from "
                            f"{_text(existing_room.get('name'))} to {endpoint.name}; "
                            f"its calculated route is {route_length:.2f} m against "
                            f"the configured {max_copper_m:.2f} m copper limit."
                        )
                        endpoint_room[endpoint.name] = selected
                        continue
                    reachable = []
                    for candidate in comms_rooms:
                        if (
                            same_floor_only
                            and _int(candidate.get("floor")) != endpoint.floor
                        ):
                            continue
                        try:
                            candidate_length, _ = endpoint_route(candidate, endpoint)
                        except NetworkPlanningError:
                            continue
                        candidate_length += endpoint.extension_distance_m
                        if candidate_length <= max_copper_m:
                            reachable.append((candidate, candidate_length))
                    if reachable:
                        selected = min(
                            reachable,
                            key=lambda row: (
                                0
                                if not _is_distributed_equipment_room(row[0])
                                else 1,
                                row[1],
                                _text(row[0].get("name")),
                            ),
                        )[0]
                    else:
                        fallback_rooms = comms_rooms
                        if same_floor_only:
                            fallback_rooms = [
                                room
                                for room in comms_rooms
                                if _int(room.get("floor")) == endpoint.floor
                            ]
                        selected = _nearest_location(endpoint, fallback_rooms)
            if selected is None:
                raise NetworkPlanningError(
    ...
    def claim_existing_der(
        ...
        for candidate in existing_der_pool:
            ...
            for endpoint_name in endpoint_names:
                ...
                try:
                    if same_floor_only:
                        route_length, _path = builder.graph.route_on_floor(
                            candidate_name, endpoint_name, endpoint.floor
                        )
                    else:
    ...
    stack_plan_kwargs = {
        ...
        "stacking_enabled": bool(
            builder.settings.get("access_stacking_enabled", True)
        ),
        "configured_max_stack_members": max(
            1, _int(builder.settings.get("access_stack_max_members"), 8)
        ),
    }
        ...
        stack_plan_kwargs.update(
            {
                ...
                # The standard access layout uses two 0.5U 24-port panels
                # around each 1U switch.  Spare capacity is held as free ports;
                # it must not consume a logical stack-member position or a
                # separate cable-manager U.
                "copper_panel_u": (
                    0.5
                    if max(1, _int(panel_asset.get("number_of_ports"), 48)) <= 24
                    else float(_copper_patch_panel_rack_units(panel_asset))
                ),
                "cable_manager_u": 0.0,
    ...
    for room_name in room_order:
        ...
        cabinet_type = _text(room.get("cabinet_type")).lower() or "standard"
        is_distributed_equipment_room = _is_distributed_equipment_room(room)
        max_switches_per_cabinet = (
            2 if cabinet_type == "slim_wall" or is_distributed_equipment_room else 0
        )
        maximum_location_cabinets = max(
            0, _int(room.get("max_network_cabinets"), 0)
        )
        room_stack_plan_kwargs = {
            **stack_plan_kwargs,
        ...
        full_stack_groups = _stack_group_plan(
            full_spare_mix, rack_size_u, **room_stack_plan_kwargs
        )
        ...
        calculate_base_mix = (
            spare_fraction > 0.0
            and (
                is_distributed_equipment_room
                or spare_mode == "defer"
                or len(full_stack_groups) > 1
            )
        )
        base_mix = access_mix(0.0) if calculate_base_mix else list(full_spare_mix)
        base_stack_groups = _stack_group_plan(
            base_mix, rack_size_u, **room_stack_plan_kwargs
        )
        ...
        if (
            spare_requires_extra_stack
            and not spare_mode
            and not is_distributed_equipment_room
        ):
            max_members = max(
                (
        ...
        # A DER is a fixed two-switch equipment-room unit. Consume all of that
        # physical capacity before creating an adjacent DER; the project-wide
        # spare percentage must not create a partly used overflow room.
        use_current_demand_capacity = (
            is_distributed_equipment_room or spare_mode == "defer"
        )
        local_spare_fraction = (
            0.0 if use_current_demand_capacity else spare_fraction
        )
        mix = (
            list(base_mix)
            if use_current_demand_capacity
        ...
        requested_additional_ders = max(
            0,
            _int(
                overrides.get("additional_der_by_location", {}).get(
                    room_name, 0
                )
            ),
        )
        if is_distributed_equipment_room and requested_additional_ders:
            # This is deliberately relative to the freshly optimised mix.  An
            # absolute minimum (for example 3) has no effect when the planner
            # already selected more devices elsewhere in a large DER design,
            # so the retry would reproduce the identical constraint.
            minimum_switches = max(
                minimum_switches, len(mix) + requested_additional_ders
            )
        if minimum_switches > len(mix):
            requested_der_asset_id = _text(
                overrides.get("additional_der_asset_by_location", {}).get(
                    room_name, ""
                )
            )
            requested_der_assets = [
                asset
                for asset in room_candidates
                if _text(asset.get("id")) == requested_der_asset_id
            ]
            padding_asset = (
                requested_der_assets[0]
                if requested_der_assets
                else min(
                    room_candidates,
                    key=lambda asset: (
                        _asset_preference_rank(asset),
        ...
        packed = _pack_ports_to_devices(
            ...
            mix,
            local_spare_fraction,
            reserve_ports_per_device=(
                is_distributed_equipment_room
                and requested_additional_ders > 0
            ),
            # Each consecutive pair is one DER. Prefer any compatible capacity
            # in that pair before assigning endpoints to the next room.
            fill_device_groups=2 if is_distributed_equipment_room else 0,
        )
        stack_groups = _stack_group_plan(
            mix, rack_size_u, **room_stack_plan_kwargs
        )
        ...
        for group_index, group in enumerate(stack_groups):
            ...
            if (
                is_distributed_equipment_room
                and der_location_switch_count > 0
            ...
            stack_topology = _text(
                builder.settings.get("access_stack_topology")
            ).lower()
            if stack_topology not in {"ring", "chain"}:
                stack_topology = "ring"
            instance_name = (
                f"AUTO {active_room_name} Access Stack {stack_number}"
                if is_stack
                else f"AUTO {active_room_name} Access Switch {switch_number}"
            )
            ...
            if (
                max_switches_per_cabinet > 0
                and rack_switch_count + member_count > max_switches_per_cabinet
            ):
                rack_index += 1
                next_rack_u = 1
                rack_switch_count = 0

            if maximum_location_cabinets and rack_index > maximum_location_cabinets:
                switch_capacity_cabinets = (
                    int(math.ceil(len(mix) / float(max_switches_per_cabinet)))
                    if max_switches_per_cabinet > 0
                    else 0
                )
                required_cabinets = max(rack_index, switch_capacity_cabinets)
                raise NetworkPlanningError(
                    f"{active_room_name} is limited to {maximum_location_cabinets} "
                    f"{('slim wall ' if cabinet_type == 'slim_wall' else '')}"
                    f"network cabinet(s), but {len(mix)} access switch(es) require "
                    f"at least {required_cabinets} cabinets.",
                    code="location_cabinet_limit",
                    details={
                        "location_name": active_room_name,
                        "maximum_cabinets": maximum_location_cabinets,
                        "required_cabinets": required_cabinets,
                        "required_switches": len(mix),
                        "maximum_switches_per_cabinet": max_switches_per_cabinet,
                        "cabinet_type": cabinet_type,
                    },
                )

            rack_base = (
                f"AUTO-RACK-{active_room_name}-TOR"
                if rack_deployment_model == "top_of_rack"
                else f"AUTO-RACK-{active_room_name}-EOR"
            )
            rack_name = (
            ...
            instance = builder.add_instance(
                ...
                stack_topology=stack_topology if is_stack else "",
                stack_interconnect_count=(
                    member_count
                    if is_stack and stack_topology == "ring"
                    else max(0, member_count - 1) if is_stack else 0
                ),
                stack_interconnect_medium="stacking",
                stack_interconnect_specification=(
                    (
                        _text(asset.get("stacking_technology"))
                        or f"{stack_topology.title()} switch-stack interconnect"
                    )
                    if is_stack
                    else ""
                ),
            ...
            for member_offset, assigned in enumerate(
                ...
                for port_number, item in enumerate(assigned, start=1):
                    endpoint = endpoint_by_name.get(item.endpoint_name)
                    if same_floor_only and endpoint is not None:
                        length_m, path = builder.graph.route_on_floor(
                            active_room_name, item.endpoint_name, endpoint.floor
                        )
                    else:
                        length_m, path = builder.graph.route(
                            active_room_name, item.endpoint_name
                        )
                    ...
                    builder.add_assignment(
                        ...
                        length_m,
                        path,
                        source_location=active_room_name,
...
def _install_external_network_connections(builder: DesignBuilder, spare_fraction: float) -> None:
    ...
    ignore_bandwidth_shortfall = bool(
        builder.settings.get("ignore_link_bandwidth_errors", False)
    )
    if (
        ...
        message = (
            "The enabled external internet services provide "
            f"{total_service_capacity:.3f} Mbps but "
            f"{required_edge_bandwidth:.3f} Mbps of north-south capacity is "
            "required including spare capacity."
        )
        if ignore_bandwidth_shortfall:
            builder.warnings.append(
                "North-south traffic capacity warning: "
                f"{required_edge_bandwidth:.3f} Mbps is required, but the enabled "
                f"external internet services provide {total_service_capacity:.3f} Mbps "
                f"({required_edge_bandwidth - total_service_capacity:.3f} Mbps shortfall). "
                "The user accepted the bandwidth-shortfall override."
            )
        else:
            raise NetworkPlanningError(
                message
                + " Increase service capacity, reduce the spare allowance, revise "
                "endpoint usage profiles, or accept the bandwidth-shortfall override.",
                code="external_north_south_capacity",
                details={
                    "required_bandwidth_mbps": required_edge_bandwidth,
                    "max_compatible_capacity_mbps": total_service_capacity,
                    "shortfall_mbps": required_edge_bandwidth
                    - total_service_capacity,
                    "capacity_scope": "external_services",
                },
            )

    def suitable_edge_asset(asset: dict, *, enforce_bandwidth: bool = True) -> bool:
        ports = max(
            _int(asset.get("number_of_ports")),
    ...
    relaxed_router_candidates = _candidate_assets(
        builder.data,
        "network_router",
        lambda asset: suitable_edge_asset(asset, enforce_bandwidth=False),
    )
    relaxed_router_candidates.extend(
        _candidate_assets(
            builder.data,
            "firewall",
            lambda asset: suitable_edge_asset(asset, enforce_bandwidth=False),
    ...
    if not relaxed_router_candidates:
        raise NetworkPlanningError(
            "External networks are configured but no router/firewall asset has "
            f"at least {required_edge_ports} usable ports and enough declared packet "
            "throughput. Add suitable edge hardware or change the model selected "
            "in the automatic-planner wizard."
        )
    router_candidates = [
        asset
        for asset in relaxed_router_candidates
        if suitable_edge_asset(asset, enforce_bandwidth=True)
    ]
    if not router_candidates:
        maximum_router_capacity = max(
            max(0.0, _float(asset.get("bandwidth_capacity_gbps"))) * 1000.0
            for asset in relaxed_router_candidates
        )
        if ignore_bandwidth_shortfall:
            router_candidates = relaxed_router_candidates
        else:
            raise NetworkPlanningError(
                "External networks are configured but no router/firewall asset has "
                f"enough declared capacity for {required_edge_bandwidth:.3f} Mbps "
                "north-south traffic. Add suitable edge hardware, change the model "
                "selected in the automatic-planner wizard, or accept the "
                "bandwidth-shortfall override.",
                code="external_north_south_capacity",
                details={
                    "required_bandwidth_mbps": required_edge_bandwidth,
                    "max_compatible_capacity_mbps": maximum_router_capacity,
                    "shortfall_mbps": max(
                        0.0, required_edge_bandwidth - maximum_router_capacity
                    ),
                    "capacity_scope": "edge_router",
                },
            )
    ...
    if cores:
        link_compatible_routers = []
        edge_alternatives = []
        paired_cores = [
            cores[index % len(cores)]
            for index in range(required_router_count)
        ]
        for asset in router_candidates:
            capacities = []
            for core in paired_cores:
                fake_router = {
                    "id": f"__EDGE-CAPACITY-{_text(asset.get('id'))}",
                    "asset_id": _text(asset.get("id")),
                    "network_layer": "edge",
                    "design_role": "edge_router",
                }
                info = builder.connection_bundle_capabilities(
                    fake_router,
                    "Downlink-LAN-1",
                ...
                capacities.append(
                    max(
                        (
                            _int(row.get("capacity_mbps"))
                            for row in info.get("capabilities", [])
                        ),
                        default=0,
                    )
                )
            compatible_capacity = min(capacities, default=0)
            edge_alternatives.append(
                {
                    "asset_id": _text(asset.get("id")),
                    "name": _text(asset.get("name")) or _text(asset.get("id")),
                    "capacity_mbps": compatible_capacity,
                    "meets_required": compatible_capacity + 1e-9
                    >= required_edge_bandwidth,
                }
            )
            if compatible_capacity + 1e-9 >= required_edge_bandwidth:
                link_compatible_routers.append(asset)
        if link_compatible_routers:
            router_candidates = link_compatible_routers
        elif not ignore_bandwidth_shortfall:
            maximum_capacity = max(
                (row["capacity_mbps"] for row in edge_alternatives), default=0
            )
            raise NetworkPlanningError(
                "No configured edge router or firewall has a speed-compatible "
                f"core-facing bundle for {required_edge_bandwidth:.3f} Mbps.",
                code="edge_core_link_capacity",
                details={
                    "required_bandwidth_mbps": required_edge_bandwidth,
                    "max_compatible_capacity_mbps": maximum_capacity,
                    "shortfall_mbps": max(
                        0.0, required_edge_bandwidth - maximum_capacity
                    ),
                    "edge_alternatives": edge_alternatives,
                },
            )
    locations_by_name = {
        _text(row.get("name")): row
    ...
    # Edge equipment belongs with the core layer, not in an arbitrary comms
    # room or carrier demarcation.  Use the actual generated core locations in
    # preference to every other root candidate; these are normally MERs.
    roots: List[dict] = []
    seen_root_names: Set[str] = set()
    for core in cores:
        root_name = _text(core.get("route_anchor")) or _text(
            core.get("location_name")
        )
        root = locations_by_name.get(root_name)
        if root is not None and root_name not in seen_root_names:
            roots.append(root)
            seen_root_names.add(root_name)
    if not roots:
        roots = _locations_by_kind(builder.data, {"mer"})
    if not roots:
        roots = _locations_by_kind(builder.data, {"comms_room", "telco_pop"})
    if not roots:
    ...
    selected_router_capacity = max(
        0.0, _float(best_router.get("bandwidth_capacity_gbps")) * 1000.0
    )
    if (
        ignore_bandwidth_shortfall
        and selected_router_capacity > 0.0
        and selected_router_capacity + 1e-9 < required_edge_bandwidth
    ):
        builder.warnings.append(
            "North-south traffic capacity warning: selected edge router/firewall "
            f"'{_text(best_router.get('name')) or _text(best_router.get('id'))}' "
            f"provides {selected_router_capacity:.3f} Mbps, but "
            f"{required_edge_bandwidth:.3f} Mbps is required including spare capacity "
            f"({required_edge_bandwidth - selected_router_capacity:.3f} Mbps shortfall). "
            "The user accepted the bandwidth-shortfall override."
        )
    routers: List[dict] = []
    router_core_pairs: List[Tuple[dict, Optional[dict]]] = []
    for index in range(required_router_count):
        local_core = cores[index % len(cores)] if cores else None
        root_name = (
            _text(local_core.get("route_anchor"))
            or _text(local_core.get("location_name"))
            if local_core is not None
            else ""
        )
        root = locations_by_name.get(root_name) or roots[index % len(roots)]
        core_rack_name = (
            _text(
                local_core.get("target_rack_name")
                or local_core.get("rack_name")
            )
            if local_core is not None
            else ""
        )
        router = builder.add_instance(
            ...
            root,
            "edge_router",
            rack_name=core_rack_name or f"AUTO-RACK-{_text(root.get('name'))}",
            rack_start_u=index + 1,
            rack_size_u=max(1, _int(builder.settings.get("default_rack_size_u"), 42)),
            route_anchor=_text(root.get("name")),
            network_layer="edge",
            associated_core_instance_id=(
                _text(local_core.get("id")) if local_core is not None else ""
            ),
            expected_north_south_bandwidth_mbps=round(
        ...
        router_core_pairs.append((router, local_core))

    if cores:
        for index, (router, paired_core) in enumerate(router_core_pairs):
            core = paired_core or cores[index % len(cores)]
...
def _install_fibre_patch_panels(builder: DesignBuilder, spare_fraction: float) -> None:
    ...
    def panel_pool_key(instance: dict) -> Optional[Tuple[int, str, str]]:
        ...
        if key is None:
            return None
        if tor_local_fibre and not tor_allow_adjacent:
            return key
        return key[0], key[1], ""
    ...
    for logical, a, b in eligible:
        ...
        # Front-panel occupancy represents live LC/SC connections, not dark
        # spare cores in the fixed cable.  Spare cores remain designated on the
        # physical cable and can be terminated later without making apparently
        # unconnected panel ports look occupied today.
        connector_positions = active_positions
        connection_plan[logical_id] = {
            ...
            "cassette_lane": (
                "failover"
                if bool(logical.get("standby"))
                or _text(logical.get("redundancy_role")).lower()
        ...
        *,
        ...
        while remaining > 0:
            ...
            for panel in ordered_panels:
                ...
                cassette_indices = list(range(cassette_count))
                if cassette_assignment_key:
                    cassette_indices.sort(
                        key=lambda index: (
                            0
                            if _text(cassette_rows[index].get("cassette_assignment_key"))
                            == cassette_assignment_key
                            and _text(cassette_rows[index].get("redundancy_role"))
                            == cassette_lane
                            else 1
                            if not _text(cassette_rows[index].get("cassette_assignment_key"))
                            else 2,
                            index,
                        )
                    )
                for cassette_index in cassette_indices:
                    ...
                    existing_lane = _text(cassette.get("redundancy_role"))
                    assignment_compatible = (
                        not cassette_assignment_key
                        or not existing_assignment
                        or (
                            existing_assignment == cassette_assignment_key
                            and existing_lane == cassette_lane
                        )
                    )
                    compatible = (
                        assignment_compatible
                    ...
                    if cassette_assignment_key:
                        cassette["cassette_assignment_key"] = cassette_assignment_key
                        cassette["redundancy_role"] = cassette_lane
    ...
    for logical, a, b in eligible:
        ...
        cassette_lane = plan["cassette_lane"]

        def access_cassette_key(instance: dict) -> str:
            return (
                "access-uplinks"
                if _text(instance.get("design_role")) == "access_switch"
                else ""
        ...
        if rack_key(a) is not None:
            a_allocations = allocate(
                ...
                termination_mode=termination_mode,
                rear_connector=rear_connector,
                cassette_assignment_key=access_cassette_key(a),
                cassette_lane=cassette_lane,
            )
        ...
        if rack_key(b) is not None:
            ...
            b_allocations = allocate(
                ...
                termination_mode=termination_mode,
                rear_connector=rear_connector,
                cassette_assignment_key=access_cassette_key(b),
                cassette_lane=cassette_lane,
...
def _copper_patch_panel_asset(builder: DesignBuilder) -> dict:
    ...
    # The standard access-switch dressing uses two 0.5U 24-port panels around
    # each 48-port switch.  Prefer an actual 24-port library asset; a 48-port
    # 1U rail cannot represent that physical layout.
    candidates = [
        asset for asset in candidates
        if _int(asset.get("number_of_ports")) == 24
    ]
    ...
    if candidates:
        ...
        selected["rack_units"] = 1
        selected["rack_height_half_u"] = 1
        return selected
    asset = {
        "id": "AUTO-COPPER-PATCH-PANEL-24-0.5U",
        "name": "0.5U 24-port Category 6A patch panel",
        "asset_type": "patch_panel",
        "patch_panel_type": "copper",
        "number_of_ports": 24,
        "connections_in": 24,
        "connections_out": 24,
        "port_definitions": [{"port_type": "rj45", "port_count": 24, "port_use": "patch", "name_prefix": "Port"}],
        "input_connection_type": "copper",
        "output_connection_type": "copper",
        "rack_units": 1,
        "rack_height_half_u": 1,
        "power_input_w": 0.0,
...
def _install_copper_patch_panels(builder: DesignBuilder, spare_fraction: float) -> None:
    ...
    locations.update(
        {
            _text(row.get("name")): row
            for row in builder.locations
            if isinstance(row, dict) and _text(row.get("name"))
        }
    )
    panel_asset = _copper_patch_panel_asset(builder)
    capacity = max(1, _int(panel_asset.get("number_of_ports"), 24))
    ...
    for (_floor, location_name), switches in sorted(switches_by_location.items()):
        ...
        if remaining_panels > 0:
            location = locations.get(location_name, {})
            is_der = _is_distributed_equipment_room(location)
            parent_location = _text(
                location.get("overflow_parent_location")
            ) or location_name
            current_switch_count = sum(
                max(1, _int(row.get("stack_member_count"), 1))
                if bool(row.get("logical_stack"))
                else 1
                for row in switches
            )
            reference_switch_asset = builder._asset_for_instance(switches[0])
            panels_per_switch = max(
                1,
                int(
                    math.ceil(
                        len(_asset_endpoint_port_slots(reference_switch_asset))
                        / float(capacity)
                    )
                ),
            )
            additional_switches = max(
                1, int(math.ceil(remaining_panels / float(panels_per_switch)))
            )
            issue = {
                    "location_name": location_name,
                    "parent_location_name": parent_location,
                    "location_kind": _text(location.get("kind")),
                    "required_port_count_with_spare": required_positions,
                    "available_port_count": sum(
                        maximum_panel_counts.values()
                    )
                    * capacity,
                    "remaining_panel_count": remaining_panels,
                    "patch_panel_capacity": capacity,
                    "current_location_switch_count": current_switch_count,
                    "suggested_device_count": current_switch_count
                    + additional_switches,
                    "switch_asset_id": _text(switches[0].get("asset_id")),
                    "switch_asset_name": _text(
                        builder._asset_for_instance(switches[0]).get("name")
                    )
                    or _text(switches[0].get("asset_id")),
                }
            if is_der:
                # Continue checking the remaining generated DERs in this same
                # pass.  Returning every matching fault together avoids a full
                # building-wide planner restart for DER1, DER2, DER3, etc.
                der_capacity_issues.append(issue)
                continue
            raise NetworkPlanningError(
    ...
    if der_capacity_issues:
        locations_text = ", ".join(
            _text(issue.get("location_name"))
            for issue in der_capacity_issues[:6]
        )
        if len(der_capacity_issues) > 6:
            locations_text += f" and {len(der_capacity_issues) - 6} more"
        raise NetworkPlanningError(
            f"{len(der_capacity_issues)} DER location(s) need adjacent capacity: "
            f"{locations_text}.",
            code="der_patch_panel_capacity_batch",
            details={
                "issues": der_capacity_issues,
                "issue_count": len(der_capacity_issues),
            },
        )
...
def _repack_generated_racks(builder: DesignBuilder) -> None:
    ...
    for (floor, location_name), items in sorted(groups.items()):
        ...
        cabinet_type = _text(location.get("cabinet_type")).lower() or "standard"
        max_switches_per_cabinet = 2 if cabinet_type == "slim_wall" else 0
        maximum_location_cabinets = max(
            0, _int(location.get("max_network_cabinets"), 0)
        )
        ...
        def create_rack(preferred_name: str = "") -> dict:
            ...
            if maximum_location_cabinets and len(racks) > maximum_location_cabinets:
                raise NetworkPlanningError(
                    f"{location_name} is limited to {maximum_location_cabinets} "
                    f"network cabinet(s), but the generated equipment requires "
                    f"at least {len(racks)}.",
                    code="location_cabinet_limit",
                    details={
                        "location_name": location_name,
                        "maximum_cabinets": maximum_location_cabinets,
                        "required_cabinets": len(racks),
                        "maximum_switches_per_cabinet": max_switches_per_cabinet,
                        "cabinet_type": cabinet_type,
                    },
                )
        ...
        def choose_bottom(
            ...
            powered: bool,
            preferred: str = "",
            switch_members: int = 0,
        ) -> dict:
            if (
                max_switches_per_cabinet > 0
                and switch_members > max_switches_per_cabinet
            ):
                raise NetworkPlanningError(
                    f"A slim wall cabinet at {location_name} can hold at most "
                    f"{max_switches_per_cabinet} switches, but "
                    f"{switch_members} stacked switch members must stay together.",
                    code="slim_wall_cabinet_capacity",
                    details={
                        "location_name": location_name,
                        "maximum_switches": max_switches_per_cabinet,
                        "required_switches": switch_members,
                        "cabinet_type": cabinet_type,
                    },
                )
            candidates = []
            if preferred and preferred in rack_by_name:
                candidates.append(create_rack(preferred))
            elif preferred and not racks:
                # Preserve the first planned cabinet name, then fill that
                # cabinet before honouring later per-stack placeholder names.
                candidates.append(create_rack(preferred))
            candidates.extend(r for r in racks if r not in candidates)
            for rack in candidates:
                if (
                    max_switches_per_cabinet > 0
                    and switch_members > 0
                    and rack["switch_count"] + switch_members
                    > max_switches_per_cabinet
                ):
                    continue
                ...
                if prospective + units - 1 <= rack["top_next"]:
                    return rack
            rack = create_rack(preferred if preferred and preferred not in rack_by_name else "")
        ...
        for instance in equipment:
            ...
            switch_members = (
                max(1, _int(instance.get("stack_member_count"), 1))
                if "switch" in _text(instance.get("design_role")).lower()
                else 0
            )
            compact_copper_layout = bool(
                switch_members
                and copper_rows
                and all(
                    max(
                        0,
                        _int(
                            assets.get(_text(panel.get("asset_id")), {}).get(
                                "rack_height_half_u"
                            ),
                            _int(panel.get("rack_height_half_u")),
                        ),
                    )
                    == 1
            ...
            compact_bundle_u = int(
                math.ceil((switch_members * 2 + len(copper_rows)) / 2.0)
            )
            rack = choose_bottom(
                (
                    compact_bundle_u
                    if compact_copper_layout
                    else (units + copper_bundle_u if tor_atomic else units)
                ),
                powered,
                preferred,
                switch_members,
            ...
            if compact_copper_layout:
                # Lay out each physical member independently while retaining a
                # single logical stack for capacity, ports and topology:
                # 0.5U panel / 1U switch / 0.5U panel.
                if powered:
                    ensure_ups(rack)
                if rack["bottom_next"] + compact_bundle_u - 1 > rack["top_next"]:
                    raise NetworkPlanningError(
                        f"Rack {rack['name']} at {location_name} has insufficient "
                        "space for the access-switch patching layout."
                    )
                base_u = rack["bottom_next"]
                half_cursor = (base_u - 1) * 2 + 1
                remaining_panels = iter(copper_rows)
                member_half_slots: List[int] = []

                def place_half_panel(panel: dict, half_slot: int) -> None:
                    panel["rack_name"] = rack["name"]
                    panel["target_rack_name"] = rack["name"]
                    panel["rack_start_u"] = (half_slot + 1) // 2
                    panel["rack_start_half_u"] = half_slot
                    panel["rack_height_half_u"] = 1
                    panel["rack_size_u"] = rack_size_u
                    panel["rack_deployment_model"] = rack_deployment_model
                    panel["cabinet_type"] = cabinet_type

                for _member in range(switch_members):
                    lower_panel = next(remaining_panels, None)
                    if lower_panel is not None:
                        place_half_panel(lower_panel, half_cursor)
                        half_cursor += 1
                    member_half_slots.append(half_cursor)
                    half_cursor += 2
                    upper_panel = next(remaining_panels, None)
                    if upper_panel is not None:
                        place_half_panel(upper_panel, half_cursor)
                        half_cursor += 1
                for panel in remaining_panels:
                    place_half_panel(panel, half_cursor)
                    half_cursor += 1

                instance["rack_name"] = rack["name"]
                instance["rack_start_u"] = (member_half_slots[0] + 1) // 2
                instance["rack_start_half_u"] = member_half_slots[0]
                instance["rack_height_half_u"] = 2
                instance["stack_member_start_half_us"] = member_half_slots
                instance["rack_size_u"] = rack_size_u
                instance["rack_deployment_model"] = rack_deployment_model
                instance["cabinet_type"] = cabinet_type
                instance.setdefault("rack_function", _text(instance.get("design_role")))
                rack["bottom_next"] = base_u + compact_bundle_u
                rack["switch_count"] += switch_members
                if powered:
                    rack["powered_items"].append(instance)
            else:
                place_bottom(instance, rack, units, powered, switch_members)
            ...
            for panel_index, panel in enumerate(
                [] if compact_copper_layout else copper_rows,
                start=1,
            ):
                panel_u = max(1, _int(panel.get("_calculated_rack_units"), 1))
...
def auto_connect_manual_devices(
    ...
    def connection_plan(device_id: str) -> Optional[dict]:
        ...
        if layer in {"access", "aggregation"}:
            ...
            allowed_sources = (
                {"aggregation", "core"} if layer == "access" else {"core"}
            )
            configured_source = _text(
                (rule or {}).get("source_layer")
            ).lower()
            if configured_source not in allowed_sources:
                if layer == "access":
                    configured_source = (
                        "aggregation"
                        if any(value == "aggregation" for value in layers.values())
                        and _text(settings.get("topology_model")).lower()
                        == "three_tier"
                        else "core"
                    )
                else:
                    configured_source = "core"
            if configured_source:
                return {
                    "source_layer": configured_source,
                    "target_layer": layer,
                    "links": max(
                        1, _int((rule or {}).get("links_per_target"), 1)
                    ),
                    "distinct": max(
                        1,
                        _int(
                            (rule or {}).get("minimum_distinct_sources"), 1
                        ),
                    ),
        ...
        if layer == "olt":
            return {"source_layer": "core", "target_layer": "olt", "links": 1, "distinct": 1, "direction": "candidate_to_device"}
        if layer == "splitter":
...
def estimate_network_switch_counts(data: dict) -> dict:
    """Return a fast minimum switch estimate for each Traditional layer.

    The estimate uses the configured asset/model preferences, endpoint port and
    PoE demand, spare capacity, access stacking policy, layer link counts and
    source-diversity rules.  Final routing, per-room cabinet limits and physical
    patching can increase the generated quantities, so the result is presented
    as an estimate rather than a committed design.
    """

    working = deepcopy(data)
    ensure_network_schema(working)
    ...
    if technology.lower() != "traditional":
        return {
            ...
            "core_switches": 0,
            "aggregation_switches": 0,
            "access_switches": 0,
            "access_stacks": 0,
            "endpoint_ports": 0,
            "note": "Core/aggregation/access switch estimation applies to Traditional designs.",
        }

    endpoints, warnings = build_endpoint_demands(working)
    port_groups: Dict[str, List[PortDemand]] = defaultdict(list)
    for endpoint in endpoints:
        group_name = _text(endpoint.existing_comms_room) or f"Floor {endpoint.floor}"
        port_groups[group_name].extend(endpoint.ports)

    access_candidates = _candidate_assets(
        working,
        "network_switch",
        lambda asset: _text(asset.get("output_connection_type")).lower() == "copper"
        and _int(asset.get("number_of_ports")) > 0
    ...
    if not access_candidates:
        raise NetworkPlanningError(
            "No copper-output access switch is available for the layer estimate."
        )
    access_candidates = _apply_manufacturer_preference(
        working, access_candidates, "access_switch", "access switch"
    )
    access_candidates = [_endpoint_capacity_asset(asset) for asset in access_candidates]

    spare_fraction = max(
        0.0, _float(settings.get("spare_capacity_percent"), 15.0)
    ) / 100.0
    rack_size_u = max(1, _int(settings.get("default_rack_size_u"), 42))
    stacking_enabled = bool(settings.get("access_stacking_enabled", True))
    stack_limit = max(1, _int(settings.get("access_stack_max_members"), 8))
    minimum_by_location = _planner_resolution_overrides(settings).get(
        "minimum_access_switches_by_location", {}
    )
    locations_by_name = {
        _text(location.get("name")): location
        for location in working.get("locations", [])
        if isinstance(location, dict) and _text(location.get("name"))
    }
    ...
    total_bandwidth = 0.0
    total_packets = 0.0
    for group_name, ports in sorted(port_groups.items()):
        port_count = len(ports)
        poe_w = sum(max(0.0, item.poe_power_w) for item in ports)
        bandwidth = sum(max(0.0, item.expected_bandwidth_mbps) for item in ports)
        packets = sum(max(0.0, item.expected_packet_rate_pps) for item in ports)
        mix = _minimum_asset_mix(
            access_candidates,
            port_count,
            poe_w,
            spare_fraction,
            f"access switch estimate at {group_name}",
            endpoint_ports_only=True,
        )
        mix = _ensure_aggregate_traffic_capacity(
            access_candidates,
        ...
        minimum_members = max(
            0, _int(minimum_by_location.get(group_name), 0)
        )
        if minimum_members > len(mix):
            padding = min(
                access_candidates,
                key=lambda asset: (
                    _asset_preference_rank(asset),
            ...
            mix.extend([padding] * (minimum_members - len(mix)))
        access_members += len(mix)
        location = locations_by_name.get(group_name, {})
        max_switches_per_cabinet = (
            2
            if _text(location.get("cabinet_type")).lower() == "slim_wall"
            or _is_distributed_equipment_room(location)
            else 0
        )
        access_stacks += len(
            _stack_group_plan(
                ...
                configured_max_stack_members=stack_limit,
                max_switches_per_cabinet=max_switches_per_cabinet,
            )
        )
    ...
    topology = _text(settings.get("topology_model")).lower()
    if topology not in {"collapsed_core", "three_tier"}:
        topology = "collapsed_core"
    required_pairs = (
        {("core", "aggregation"), ("aggregation", "access")}
        if topology == "three_tier"
        else {("core", "access")}
    )
    current_rules = normalise_layer_connection_rules(
        settings.get("layer_connection_rules"),
        topology,
        bool(settings.get("redundant_core", True)),
        _int(settings.get("independent_link_count"), 2),
    ...
    if not required_pairs.issubset(current_pairs):
        current_rules = normalise_layer_connection_rules(
            [],
            topology,
            bool(settings.get("redundant_core", True)),
    ...
    settings["layer_connection_rules"] = current_rules

    def estimate_layer(
        candidates: Sequence[dict],
        target_count: int,
        rule: Optional[dict],
        peer_rule: Optional[dict],
        label: str,
        per_device_ports: int = 0,
    ) -> int:
        if not candidates or target_count <= 0 or rule is None:
            return 0
        capacity_candidates = _layer_capacity_candidates(candidates)
        links = max(1, _int(rule.get("links_per_target"), 1))
        fixed_ports = target_count * links
        minimum = max(1, _int(rule.get("minimum_distinct_sources"), 1))
        peer_links = max(
            0, _int((peer_rule or {}).get("links_per_target"), 0)
        )
        mix = _minimum_layer_mix(
            capacity_candidates,
            fixed_ports,
            per_device_ports,
            peer_links,
            ...
            total_bandwidth,
            total_packets,
            bundle_port_demands=[1] * fixed_ports,
        )
        return len(mix)
    ...
    if topology == "three_tier":
        ...
        core_aggregation = _layer_rule(settings, "core", "aggregation")
        aggregation_peer = _layer_rule(settings, "aggregation", "aggregation")
        aggregation_count = estimate_layer(
            _choose_aggregation_candidates(working),
            access_stacks,
            aggregation_access,
            aggregation_peer,
            "aggregation switch estimate",
            per_device_ports=max(
                1, _int((core_aggregation or {}).get("links_per_target"), 1)
            ),
        )
        core_count = estimate_layer(
            _choose_core_candidates(working),
            aggregation_count,
            core_aggregation,
            core_peer,
...
def generate_network_design(
    ...
    settings.setdefault("auto_planner_same_floor_only", False)
    ...
    summary = {
        ...
        "same_floor_only": bool(
            settings.get("auto_planner_same_floor_only", False)
        ),
        ...
        "modular_chassis_configurations": [
            {
                "instance_id": _text(instance.get("id")),
                "name": _text(instance.get("name")),
                "asset_id": _text(instance.get("asset_id")),
                "network_layer": _text(instance.get("network_layer")),
                "chassis_modules": deepcopy(instance.get("chassis_modules", [])),
            }
            for instance in builder.instances
            if bool(instance.get("modular_chassis_configuration_auto", False))
        ],
