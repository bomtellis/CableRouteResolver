...
_INSTALLED_NETWORK_RECORD_KEYS = (
    "network_asset_instances",
    "network_racks",
    "network_connections",
    "network_power_connections",
...
class NetworkPlannerDialog(QDialog):
    def __init__(self, parent, data: dict, on_save: Callable[[dict], None]):
        ...
        self.clear_installed_button.setToolTip(
            "Remove the complete installed network design, whether generated by the "
            "planner or created manually. Asset libraries, usage profiles, fibre cable "
            "types and reusable planner settings are preserved."
        )
    ...
    def clear_installed_assets_and_connections(self) -> None:
        record_counts = {
            key: len(self.data.get(key, []) or [])
            for key in _INSTALLED_NETWORK_RECORD_KEYS
        }
        external_network_count = len(
            self.data.get("network_external_networks", []) or []
        )
        planner_overrides = self.data.get("network_settings", {}).get(
            "auto_planner_resolution_overrides", {}
        )
        has_design_summary = bool(self.data.get("network_design_summary", {}))

        if not any(record_counts.values()) and not external_network_count and not any(
            (
                planner_overrides,
                has_design_summary,
            )
        ...
        message = (
            "Clear the complete installed network design?\n\n"
            f"Installed assets: {record_counts['network_asset_instances']}\n"
            f"Racks: {record_counts['network_racks']}\n"
            f"Connections and power links: "
            f"{record_counts['network_connections'] + record_counts['network_power_connections']}\n"
            f"Endpoint assignments: {record_counts['network_endpoint_assignments']}\n"
            f"Optic modules: {record_counts['network_optic_modules']}\n"
            f"VLANs, routes and IP allocations: "
            f"{record_counts['network_vlans'] + record_counts['network_routes'] + record_counts['network_ip_allocations']}\n"
            f"External and peering network definitions: {external_network_count}\n"
            f"Physical fibre records: "
            f"{record_counts['network_fibre_cables'] + record_counts['network_fibre_nodes'] + record_counts['network_fibre_splices']}\n"
            "\n"
            "This removes records created manually as well as by the automatic planner. "
            "Network asset libraries, endpoint asset libraries, usage profiles, fibre "
            "cable types and reusable planner settings will remain. The routing graph, "
            "locations, departments and data points will not be changed. If external "
            "networks exist, you will be asked separately whether to keep them."
        )
        if (
            QMessageBox.question(
                self,
        ...
        keep_external_networks = True
        if external_network_count:
            keep_external_networks = (
                QMessageBox.question(
                    self,
                    "Keep external networks",
                    f"Keep {external_network_count} external/peering network "
                    "definition(s)?\n\nInstalled demarcation and peering-device links "
                    "will be detached because those installed assets are being cleared. "
                    "Provider, ASN, prefixes and other reusable definition data will remain.",
                    QMessageBox.Yes | QMessageBox.No,
                    QMessageBox.Yes,
                )
                == QMessageBox.Yes
        ...
        for key in _INSTALLED_NETWORK_RECORD_KEYS:
            self.data[key] = []
        if keep_external_networks:
            for external in self.data.get("network_external_networks", []) or []:
                if isinstance(external, dict):
                    external["demarcation_instance_id"] = ""
                    external["peer_instance_ids"] = []
        else:
            self.data["network_external_networks"] = []
        ...
        QMessageBox.information(
            ...
            + (
                f"Kept {external_network_count} external/peering network definition(s)."
                if keep_external_networks and external_network_count
                else "External/peering network definitions were also cleared."
            ),
        )
