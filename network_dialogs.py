...
def _combo_with_data(options: Sequence[Tuple[str, str]], current: str = "") -> QComboBox:
    combo = QComboBox()
...
class NetworkAssetEditorDialog(QDialog):
    def __init__(self, parent=None, asset: Optional[dict] = None, suggested_id: str = "NA1"):
        ...
        _set_combo_text(self.patch_panel_type_combo, _text(self.asset.get("patch_panel_type")))
        ...
        for frequency in FREQUENCY_OPTIONS:
            ...
            item.setCheckState(Qt.Checked if frequency in selected_frequencies else Qt.Unchecked)
        ...
        self.additional_frequencies_edit.setPlaceholderText("Other frequencies, comma separated")
        ...
        self.power_input_spin.setValue(float(self.asset.get("power_input_w", 0.0) or 0.0))
        ...
        self.number_of_ports_spin.setValue(int(self.asset.get("number_of_ports", 0) or 0))
        ...
        self.connections_out_spin.setValue(int(self.asset.get("connections_out", 0) or 0))
        ...
        _set_combo_text(self.output_type_combo, _text(self.asset.get("output_connection_type")) or "copper")

        self.uplink_type_combo = QComboBox()
        self.uplink_type_combo.addItems(CONNECTION_MEDIA)
        _set_combo_text(self.uplink_type_combo, _text(self.asset.get("uplink_connection_type")) or "fibre")
    ...
    def accept(self) -> None:
        ...
        if not name:
            ...
            return
        if asset_type == "fibre_splitter" and not self.split_ratio_combo.currentText().strip():
            QMessageBox.critical(self, "Invalid asset", "A fibre splitter requires a split ratio.")
            return
        frequencies = self._frequencies()
        if asset_type == "wireless_access_point" and not frequencies:
            QMessageBox.critical(self, "Invalid asset", "A wireless access point requires at least one frequency.")
            return
        ...
        self.result = {
            ...
            "patch_panel_type": self.patch_panel_type_combo.currentText().strip() if asset_type == "patch_panel" else "",
            "split_ratio": self.split_ratio_combo.currentText().strip() if asset_type == "fibre_splitter" else "",
...
class NetworkInstanceEditorDialog(QDialog):
    def __init__(
        ...
        for asset in sorted(self.assets, key=lambda item: (_text(item.get("asset_type")), _text(item.get("name")))):
        ...
        for location in sorted(self.locations, key=lambda item: (int(item.get("floor", 0)), _text(item.get("name")))):
        ...
        location_index = self.location_combo.findData(_text(self.instance.get("location_name")))
        if location_index >= 0:
    ...
    def _location_changed(self) -> None:
        ...
        if not name:
            return
        location = next((item for item in self.locations if _text(item.get("name")) == name), None)
        if location is None:
            return
...
class NetworkConnectionEditorDialog(QDialog):
    def __init__(
        ...
        _set_combo_text(self.role_combo, _text(self.connection.get("connection_role")) or "output")
        self.medium_combo = QComboBox()
        self.medium_combo.addItems(CONNECTION_MEDIA[:-1])
        _set_combo_text(self.medium_combo, _text(self.connection.get("medium")) or "copper")
        ...
        _set_combo_text(self.route_profile_combo, _text(self.connection.get("route_profile")))
        self.route_path_edit = QLineEdit(
            " -> ".join(str(value) for value in self.connection.get("route_path", []) if _text(value))
        )
        self.route_path_edit.setPlaceholderText("Optional graph node path, separated by ->")
    ...
    def accept(self) -> None:
        ...
        if not connection_id:
            QMessageBox.critical(self, "Invalid connection", "Connection ID is required.")
            return
        if not from_id or not to_id:
            QMessageBox.critical(self, "Invalid connection", "Select both connection endpoints.")
            return
        if from_id == to_id:
            QMessageBox.critical(self, "Invalid connection", "Connection endpoints must be different.")
            return
        if not self.from_port_edit.text().strip() or not self.to_port_edit.text().strip():
            QMessageBox.critical(self, "Invalid connection", "Both port identifiers are required.")
            return
        self.result = {
            "id": connection_id,
            ...
            "fibre_count": int(self.fibre_count_spin.value()) if self.medium_combo.currentText() == "fibre" else 0,
            "vlan_ids": self._selected_vlans(),
            "route_profile": self.route_profile_combo.currentText().strip(),
            "route_path": [part.strip() for part in self.route_path_edit.text().split("->") if part.strip()],
            "notes": self.notes_edit.toPlainText().strip(),
        }
        super().accept()
...
class VlanEditorDialog(QDialog):
    def __init__(self, parent=None, vlan: Optional[dict] = None, suggested_id: str = "VLAN1"):
        super().__init__(parent)
        ...
        for label, widget in [
            ("Record ID", self.id_edit), ("VLAN ID", self.vlan_id_spin), ("Name", self.name_edit),
            ("Purpose", self.purpose_edit), ("Subnet", self.subnet_edit), ("Gateway", self.gateway_edit),
            ("DHCP scope", self.dhcp_scope_edit), ("Security zone", self.security_zone_edit), ("Notes", self.notes_edit),
        ]:
    ...
    def accept(self) -> None:
        if not self.id_edit.text().strip() or not self.name_edit.text().strip():
            QMessageBox.critical(self, "Invalid VLAN", "Record ID and name are required.")
            return
        self.result = {
...
class RouteEditorDialog(QDialog):
    def __init__(self, parent=None, route: Optional[dict] = None, suggested_id: str = "NR1", vlans: Optional[Sequence[dict]] = None):
        super().__init__(parent)
        ...
        for vlan in sorted(vlans or [], key=lambda item: int(item.get("vlan_id", 0) or 0)):
            self.vlan_combo.addItem(f"{vlan.get('vlan_id', '')} - {_text(vlan.get('name'))}", _text(vlan.get("id")))
        ...
        _set_combo_text(self.protocol_combo, _text(self.route.get("protocol")) or "static")
        ...
        for label, widget in [
            ("Route ID", self.id_edit), ("Source", self.source_edit), ("Destination", self.destination_edit),
            ("VLAN", self.vlan_combo), ("Protocol", self.protocol_combo), ("Next hop", self.next_hop_edit),
            ("Metric", self.metric_spin), ("Firewall policy", self.firewall_policy_edit), ("Notes", self.notes_edit),
        ]:
            form.addRow(label, widget)
        buttons = QDialogButtonBox(QDialogButtonBox.Ok | QDialogButtonBox.Cancel)
        buttons.accepted.connect(self.accept)
    ...
    def accept(self) -> None:
        if not self.id_edit.text().strip() or not self.destination_edit.text().strip():
            QMessageBox.critical(self, "Invalid route", "Route ID and destination are required.")
            return
        vlan_value = _text(self.vlan_combo.currentData()) or self.vlan_combo.currentText().strip()
        self.result = {
            "id": self.id_edit.text().strip(),
    ...
    def set_rows(self, rows: Sequence[Sequence[object]]) -> None:
        ...
        for row_index, values in enumerate(rows):
            for column_index, value in enumerate(values):
                self.table.setItem(row_index, column_index, QTableWidgetItem(str(value if value is not None else "")))
...
class NetworkPlannerDialog(QDialog):
    def __init__(self, parent, data: dict, on_save: Callable[[dict], None]):
        ...
        _set_combo_text(self.technology_combo, _text(self.data.get("network_settings", {}).get("technology")) or "Traditional")
    ...
    def _replace_or_append(self, key: str, old_index: int, value: dict) -> bool:
        ...
        for index, item in enumerate(items):
            if index != old_index and _text(item.get("id")) == value_id:
                QMessageBox.critical(self, "Duplicate ID", f"ID {value_id} already exists.")
                return False
        if old_index < 0:
            items.append(value)
        else:
    ...
    def refresh_tables(self) -> None:
        ...
        self.connections_tab.set_rows([
            [item.get("id", ""), item.get("from_instance_id", ""), item.get("from_port", ""),
             item.get("to_instance_id", ""), item.get("to_port", ""), item.get("connection_role", ""),
             item.get("medium", ""), item.get("cable_specification", ""), ", ".join(item.get("vlan_ids", []))]
            for item in self._items("network_connections")
        ])
        self.vlans_tab.set_rows([
            [item.get("id", ""), item.get("vlan_id", ""), item.get("name", ""), item.get("purpose", ""),
             item.get("subnet", ""), item.get("gateway", ""), item.get("security_zone", "")]
            for item in self._items("network_vlans")
        ])
        self.routes_tab.set_rows([
            [item.get("id", ""), item.get("source", ""), item.get("destination", ""), item.get("vlan_id", ""),
    ...
    def edit_asset(self) -> None:
        ...
        if item is None:
            return
        dialog = NetworkAssetEditorDialog(self, item, suggested_id=_text(item.get("id")))
        if dialog.exec() == QDialog.Accepted and dialog.result:
    ...
    def delete_asset(self) -> None:
        ...
        if any(_text(instance.get("asset_id")) == asset_id for instance in self._items("network_asset_instances")):
            QMessageBox.critical(self, "Asset in use", "Delete installed instances that use this asset first.")
            return
        if QMessageBox.question(self, "Delete network asset", f"Delete {asset_id}?") == QMessageBox.Yes:
            self._items("network_assets").pop(index)
            self.refresh_tables()

    def add_instance(self) -> None:
        if not self._items("network_assets"):
            QMessageBox.information(self, "No network assets", "Create a network asset definition first.")
            return
    ...
    def add_connection(self) -> None:
        if len(self._items("network_asset_instances")) < 2:
            QMessageBox.information(self, "Not enough assets", "Install at least two network assets first.")
            return
    ...
    def delete_connection(self) -> None:
        index, item = self._selected(self.connections_tab, "network_connections")
        if item is not None and QMessageBox.question(self, "Delete connection", f"Delete {_text(item.get('id'))}?") == QMessageBox.Yes:
            self._items("network_connections").pop(index)
            self.refresh_tables()

    def add_vlan(self) -> None:
        dialog = VlanEditorDialog(self, suggested_id=_next_id(self._items("network_vlans"), "VLAN"))
        if dialog.exec() == QDialog.Accepted and dialog.result:
    ...
    def delete_vlan(self) -> None:
        ...
        if QMessageBox.question(self, "Delete VLAN", f"Delete {vlan_id} and remove its references?") != QMessageBox.Yes:
            return
        self._items("network_vlans").pop(index)
        for connection in self._items("network_connections"):
            connection["vlan_ids"] = [value for value in connection.get("vlan_ids", []) if _text(value) != vlan_id]
        for route in self._items("network_routes"):
    ...
    def edit_route(self) -> None:
        ...
        if item is None:
            return
        dialog = RouteEditorDialog(self, item, _text(item.get("id")), self._items("network_vlans"))
        if dialog.exec() == QDialog.Accepted and dialog.result:
            self._replace_or_append("network_routes", index, dialog.result)

    def delete_route(self) -> None:
        index, item = self._selected(self.routes_tab, "network_routes")
        if item is not None and QMessageBox.question(self, "Delete route", f"Delete {_text(item.get('id'))}?") == QMessageBox.Yes:
